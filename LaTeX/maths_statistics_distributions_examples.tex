...
The Z-score is a universal translator: it converts any raw measurement, regardless of its original units or scale, into "how many standard deviations from average." Once everything is in Z units, the same standard normal table works for heights, weights, or test scores alike.
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Distributions in Practice}
\begin{block}{Think About It}
For the level-crossing accident example ($\lambda=5$ per year), would you expect the probability of exactly 0 accidents to be higher or lower than the probability of exactly 5 accidents?
\end{block}
\pause
\begin{block}{Answer}
Lower. Since $\lambda=5$ is the average, outcomes near 5 are the most likely under a Poisson distribution -- $P(X=0) = e^{-5} \approx 0.0067$ is far smaller than $P(X=5) = \frac{e^{-5}5^5}{5!} \approx 0.175$, the peak of the distribution.
\end{block}
\end{frame}