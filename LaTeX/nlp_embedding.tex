...
\begin{frame}[fragile]\frametitle{Captures Relationship}
  \begin{itemize}
	...
	\item Once the words are vectors, their properties can be utilized.
...
\item Answer: The IDF (inverse document frequency) weights learned during training 
are frozen and applied to test data. Only the TF (term frequency) values are 
recalculated per test sample. Retraining does NOT occur.
\end{itemize}
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{frame}[fragile]\frametitle{CBOW: Architecture \& Training}
% \begin{itemize}
  % \item \textbf{Goal:} Predict target word $w_t$ from context words $\{w_{t-k},\ldots,w_{t+k}\}$
  % \item \textbf{Architecture:} Input (one-hot) $\rightarrow$ Projection (average) $\rightarrow$ Output (softmax)
  % \item \textbf{Objective:} Maximise $\displaystyle\frac{1}{T}\sum_{t=1}^{T}\log P\!\left(w_t \mid w_{t-k},\ldots,w_{t+k}\right)$
  % \item Context vectors are \textit{averaged} before predicting — faster, smoother
  % \item \textbf{Best for:} Smaller datasets, frequent words, faster training
% \end{itemize}
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{frame}[fragile]\frametitle{Skip-Gram: Architecture \& Training}
% \begin{itemize}
  % \item \textbf{Goal:} Given word $w_t$, predict each context word $w_{t+j}$
  % \item \textbf{Objective:} Maximise $\displaystyle\frac{1}{T}\sum_{t=1}^{T}\sum_{\substack{j=-k \\ j\neq 0}}^{k}\log P\!\left(w_{t+j} \mid w_t\right)$
  % \item \textbf{Negative Sampling (NS):} Replace full softmax --- sample $k$ noise words as negatives; reduces compute dramatically
  % \item Each step updates the target word vector + $k$ negative vectors only
  % \item \textbf{Best for:} Large corpora, rare words, richer semantic relations
% \end{itemize}

% \begin{lstlisting}[language=Python]
% from gensim.models import Word2Vec
% # sg=1 -> Skip-Gram, negative sampling
% model = Word2Vec(sentences, vector_size=100,
                 % window=5, sg=1, negative=5,
                 % min_count=2)
% \end{lstlisting}
% \end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{CBOW vs Skip-Gram: Quick Comparison}
\begin{center}
\begin{tabular}{lll}
\hline
...
\item Layer-wise representations (task-dependent):
  \begin{itemize}
  \item Lower layers typically capture: syntax, morphology
  \item Upper layers typically capture: semantics, word sense
  \end{itemize}
\item Note: Task-specific analysis may reveal different patterns
\end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Case Study: Tweet Sentiment with Word2Vec}
\begin{itemize}
  \item \textbf{Task:} Classify tweets as positive / negative
  \item \textbf{Pipeline:}
  \begin{itemize}
    \item Preprocess tweets (tokenise, remove noise)
    \item Train a Word2Vec model on tweet corpus (gensim)
    \item Represent each tweet as the \textit{mean} of its word vectors
    \item Train a Logistic Regression classifier (scikit-learn)
  \end{itemize}
  \item Mean-pooling is simple yet competitive for short texts
\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Tweet Sentiment: Data \& Preprocessing}
\begin{lstlisting}[language=Python]
import re, nltk
...
# Example
tweet = "Loving the new #NLP models! https://t.co/x"
print(preprocess(tweet))
# ['loving', 'the', 'new', 'nlp', 'models', '!']
\end{lstlisting}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Tweet Sentiment: Train Word2Vec}
\begin{lstlisting}[language=Python]
from gensim.models import Word2Vec

# tweets_tokens: list of token lists
model = Word2Vec(tweets_tokens,
                 vector_size=100,
                 window=5,
                 sg=1,          # Skip-Gram
                 negative=5,
...
model.save("tweet_w2v.model")
print(model.wv.most_similar("happy", topn=5))
# [('glad', 0.91), ('thrilled', 0.89), ...]
\end{lstlisting}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Tweet Sentiment: Feature Extraction \& Classification}
\begin{lstlisting}[language=Python]
import numpy as np
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import classification_report

def tweet_vector(tokens, model, dim=100):
    vecs = [model.wv[w] for w in tokens
            if w in model.wv]
    return np.mean(vecs, axis=0) if vecs \
           else np.zeros(dim)

X = np.array([tweet_vector(t, model)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Tweet Sentiment: Results \& Takeaways}
\begin{itemize}
  \item Typical accuracy on balanced tweet sets: \textbf{78--84\%}
  \item \textbf{Key observations:}
  \begin{itemize}
    \item Mean-pooling discards word order --- sufficient for sentiment
    \item Rare/slang words missed by \texttt{min\_count} degrade coverage
    \item Pre-trained Twitter Word2Vec (GloVe-Twitter-100d) often outperforms custom training on small corpora
  \end{itemize}
  \item \textbf{Easy improvements:}
  \begin{itemize}
    \item TF-IDF weighted pooling instead of simple mean
    \item Replace LR with SVM (\texttt{kernel='rbf'}) or MLP
    \item Fine-tune a pre-trained BERTweet for state-of-the-art results
  \end{itemize}
\end{itemize}
\end{frame}
