...
\begin{itemize}
\item Chi square(written "$\chi^2$") is a numerical value that measures the difference between an experiment's expected and observed values.
\item The equation for chi square is: $\chi^2 = \sum \frac{(o-e)^2}{e}$, where ``o'' is the observed value and ``e'' is the expected value
\begin{itemize}
\item $\chi^2 = \frac{(90-100)^2}{100} + \frac{(60-50)^2}{50}$
\item $\chi^2 = \frac{(-10)^2}{100} + \frac{(10)^2}{50}$
\item $\chi^2 = \frac{100}{100} + \frac{100}{50} = 1 + 2 = 3$
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: P-Values in Practice}
\begin{block}{Think About It}
"We couldn't reject the null hypothesis, so our town's police definitely aren't biased." Is this conclusion correct? Why or why not?
\end{block}
\pause
\begin{block}{Answer}
Not quite -- failing to reject $H_0$ only means the evidence wasn't strong enough to conclude a difference exists (which itself could be due to a small sample). It does NOT prove there is no bias; it just means this particular test didn't find convincing evidence of one.
\end{block}
\end{frame}
