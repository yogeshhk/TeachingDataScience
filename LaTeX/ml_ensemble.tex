...
\item Intuitive Justification: When combining multiple, independent, and diverse, decisions each of
...
\item Binomial theorem says that there are ${5 \choose 1}$ arrangements of this possible. See, the incorrect one can be any one of the 5. So the probability is ${5 \choose 1} (0.7)^4  \times (0.3)^1$
\item Probability that 3 classifiers would be correct and two incorrect is ${5 \choose 2}  (0.7)^3 \times (0.3)^2$
...
Think of each juror's mistake as an independent coin flip biased slightly towards being
correct. Pooling many such flips through a majority vote makes the rare event ``more than
half are simultaneously wrong'' vanishingly unlikely -- exactly like how averaging many
noisy measurements cancels out random error. This only works because the mistakes are
independent; correlated experts who share the same blind spot don't get this benefit.
\end{block}
\end{frame}
