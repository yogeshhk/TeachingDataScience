...
\begin{itemize}
\item A musical ensemble blends several instruments into one harmony.
\item An architectural ensemble is a set of buildings designed as a unit.
\end{itemize}
...
\begin{itemize}
\item Condorcet's jury theorem (1784) is an early result about ensembles.
\item If each juror judges independently and is right more than 50\% of the time:
\begin{itemize}
\item The whole jury's accuracy increases with more jurors, approaching 1.
\end{itemize}
\item If each juror is right less than 50\% of the time:
\begin{itemize}
\item The whole jury's accuracy decreases with more jurors, approaching 0.
\end{itemize}
\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Idea behind Ensemble: Rationale}
\begin{itemize}
...
\item Intuitive justification: combine multiple, independent, diverse decisions, each better than random guessing.
\begin{itemize}
\item Random errors cancel each other out.
\item Correct decisions get reinforced.
\end{itemize}
\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Idea behind Ensemble: Worked Example}
\begin{itemize}
...
\begin{tikzpicture}[every node/.style={font=\small}, box/.style={draw, minimum width=1.3cm, minimum height=1.3cm}]
\foreach \i in {1,...,5} {
  \node[box] (c\i) at (\i*1.9,0) {$C_{\i}$};
  \node[above=3pt of c\i] {70\%};
}
\node[below=14pt of c3, align=center, font=\footnotesize] {$A_i$ = ``classifier $i$ predicts correctly'', \ \ $P(A_i)=0.7$ \\ All five decisions are \textbf{independent}.};
\end{tikzpicture}%
}
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Idea behind Ensemble: Why the Binomial Coefficient?}
Before computing probabilities, let's understand where ${N \choose k}$ comes from, using the same 5 classifiers.
\begin{itemize}
\item Say we want $P(\text{exactly 4 correct, 1 wrong})$.
\item There isn't just one way for this to happen: the wrong one could be $C_1$, or $C_2$, or $C_3$, or $C_4$, or $C_5$ -- that's 5 distinct, mutually exclusive arrangements.
\item Each arrangement has the same probability, $(0.7)^4 \times (0.3)^1$, so we add it up 5 times.
\item ``How many ways to pick which 1 of the 5 is wrong'' is exactly what ${5 \choose 1} = 5$ counts.
\end{itemize}
\begin{block}{Intuition}
${N \choose k}$ counts ``how many different ways can I pick $k$ items out of $N$'', ignoring order. Here it counts how many different subsets of classifiers could be the wrong ones. For 2 wrong out of 5: ${5 \choose 2} = 10$, one for each pair like $\{C_1,C_2\}, \{C_1,C_3\}, \ldots$ -- listing them out and counting confirms there are exactly 10 pairs.
\end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Idea behind Ensemble: Probability Calculation}
\begin{itemize}
\item Probability that all 5 classifiers would be correct is $ (0.7) * (0.7) \ldots = (0.7)^5$

\item Probability that 4 classifiers would be correct and one incorrect is ${5 \choose 1} (0.7)^4  \times (0.3)^1$ (5 arrangements, as we just saw)
\item Probability that 3 classifiers would be correct and two incorrect is ${5 \choose 2}  (0.7)^3 \times (0.3)^2$ (10 arrangements)

\item Total probability is to add them up = $(0.7)^5 + {5 \choose 1} (0.7)^4  \times (0.3)^1 + {5 \choose 2} (0.7)^3 \times (0.3)^2 \approx 0.84$
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Jury to Ensemble?: Check Against Our Worked Example}
This general formula is just the earlier calculation written compactly. Plug in $N=5$, $p=0.7$; majority needs $m = \frac{5+1}{2} = 3$:
\begin{center}
\adjustbox{max width=\linewidth}{$\displaystyle \mu = \sum_{i=3}^{5}{5\choose i}(0.7)^i(0.3)^{5-i} = {5\choose3}(0.7)^3(0.3)^2 + {5\choose4}(0.7)^4(0.3)^1 + {5\choose5}(0.7)^5 \approx 0.84$}
\end{center}
\begin{block}{Intuition}
Same three terms, same answer, 0.84, as before -- the sum just says ``add up every way to get $m$ or more correct out of $N$''.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ensemble Rationale: Identical Classifiers}
Another Example:
\begin{itemize}
...
\item Majority wins, but since every classifier agrees, the ensemble simply inherits the same error rate: $\epsilon$= 0.35. No improvement at all.
\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ensemble Rationale: Error Probability}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ensemble Rationale: Conditions}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ensemble Meta-Algorithms}
\begin{itemize}
\item Ensembles are called ``meta-algorithms'': ways to combine several machine learning techniques into one predictive model.
\item Bagging and random forests reduce the complexity of models that overfit the training data.
\item Boosting increases the complexity of models that underfit the training data (high bias).
\end{itemize}
\end{frame}
...
\node[box] (data) {Complex\\Distribution};
\node[box, right=of data] (sample) {Sample\\$n{=}10$};
\node[box, right=of sample] (med) {Median\\$med_i$};
\node[right=of med, align=center] (rep) {$\times\,1000$\\(repeat)};
\node[box, right=of rep, align=center] (final) {Median of\\$med_1,\ldots,med_{1000}$};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Bagging: Bootstrap Distribution}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Bagging: Visual Summary}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Bagging: Training Trees}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Bagging: Prediction by Majority Vote}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Bagging: Decision Boundary Comparison}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Boosting: Basic Steps}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Boosting: Weak Learners}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Boosting: Sequential Focus}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Boosting: Subset Creation}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Boosting: Iterative Process}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Bagging (Recap): Key Properties}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Bagging (Recap): Illustrated}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Boosting (Recap): Key Properties}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Boosting (Recap): Illustrated}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ensemble Choices: Which Approach?}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ensemble Choices: Empirical Results}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ensemble Methods (Recap): Illustrated}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ensemble Methods (Recap): Pros and Cons}
\begin{itemize}
\item Advantages:
\begin{itemize}
