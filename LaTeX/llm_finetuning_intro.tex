...
\begin{frame}[fragile]\frametitle{Introduction to Fine-Tuning}
  \begin{itemize}
    ...
    \item \textbf{Example:} Healthcare organization fine-tunes GPT-3 for generating patient reports, adapting to medical terminologies.
	
	\item \textbf{Base Model:}: Are typically raw, may not give good results, but instruct-models are already tuned a bit, so they give better results. Fine-tuning either of them is ok, IMO. Instruct models come with their own prompt templates (read documentation).

    \item \textbf{Applicability Beyond LLMs:} Convolutional neural network fine-tuning for detecting trucks on highways.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]\frametitle{Why Fine-Tune Large Language Models?}
  \begin{itemize}
    ...
    \item Handle modest datasets and mitigate suboptimal performance
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{What is PEFT?}
