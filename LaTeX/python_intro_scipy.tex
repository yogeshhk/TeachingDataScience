...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: NumPy vs.\ SciPy}
\begin{block}{Intuition}
If NumPy is a box of raw building blocks (arrays and basic math), SciPy is the fully-stocked
workshop built on top of it: specialized tools for integration, optimization, interpolation,
and signal processing, each one already knowing how to work with NumPy arrays out of the box.
\end{block}
\end{frame}
...
# y2 is pendulum 2's y-coordinate from the double pendulum example above
F = fft(y2[:,0])
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: SciPy}
\begin{block}{Think About It}
Both \lstinline{quad()} (numerical integration) and \lstinline{odeint()} (solving ODEs) take a
Python \emph{function} as an argument, not just data. Why does that matter?
\end{block}
\pause
\begin{block}{Answer}
Passing a function means \lstinline{quad}/\lstinline{odeint} can evaluate it at any point they
choose -- including adaptively sampling more densely wherever the function changes quickly, to
control error. That's the difference between ``integrate this fixed table of $(x,y)$ values''
and ``integrate this mathematical function as precisely as I ask for'' -- adaptive precision
isn't possible if all you have is a static table of numbers.
\end{block}
\end{frame}
