...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Probability Distributions}
\begin{block}{Think About It}
You're counting how many typos appear per page in a book (a rare, independent event over a fixed unit). Which distribution from this deck best models that -- Bernoulli, Binomial, or Poisson?
\end{block}
\pause
\begin{block}{Answer}
Poisson -- it's built exactly for counting rare, independent occurrences per unit (of time, area, or in this case, per page). Bernoulli models a single yes/no trial, and Binomial counts successes over a fixed, known number of repeated trials -- neither fits "count of a rare event over a continuum" as naturally as Poisson.
\end{block}
\end{frame}
