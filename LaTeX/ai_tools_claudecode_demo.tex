...
\textbf{System requirements:}
  \begin{itemize}
    \item macOS 10.15+, Ubuntu 20.04+, Windows 10+ %(WSL or Git Bash)
    \item 4 GB RAM minimum, active internet connection
	\item Anthropic subscription like Pro with Claude Code 
...
\textbf{Essential Software Installations:}
  \begin{itemize}
    ...
    \item \textbf{Node.js $\geq$ 18 (required):} \lstinline|node --version| shows v18 or higher
    \item \textbf{npm installed:} \lstinline|npm --version| shows a version number
    \item \textbf{Git installed:} \lstinline|git --version| shows git version 2.x
    \item Python $\geq$ 3.10 and pip: \lstinline|python --version|
    \item Git configured: \lstinline|git config --global user.name "Your Name"|
  \end{itemize}

\textbf{Accounts \& Subscriptions:}
  \begin{itemize}
    \item \textbf{Claude Pro subscription} (required for Claude Code)
    \item \textbf{GitHub account} (for version control and deployment)
    \item \textbf{Vercel account} (use same email as GitHub for seamless integration)
    \item (Optional) Groq API key: \href{https://console.groq.com}{\texttt{console.groq.com}}
...
\textbf{Pro Tip:} Claude Code authenticates via your Anthropic account or API key directly, no separate provider selection step is required. Verify the connection: ask ``Hello, which model are you?''
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Customization}
...
## Step 2: Apply fix in normal mode:
...
## Step 3: Visualise context usage:
...
## Create a reusable /changelog command:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Create a Review Subagent}
...
## Create a reusable /review command:
...
## Non-interactive mode for CI (GitHub Actions):
...
\begin{lstlisting}
## {Auto-format Python after every file edit (PostToolUse):
{
  "hooks": {
    "PostToolUse": [
      {
...
\begin{frame}[fragile]\frametitle{}
\begin{lstlisting}
## Auto-run tests when Claude finishes (Stop hook):
...
## Desktop notifications when Claude needs you (Notification hook):
...
## Load git context at session start (SessionStart hook):
...
## Deep post-edit analysis (agent hook):
...
## Package our demo config as a plugin:
...
% ## PR-linked sessions (unique to Claude Code):
...
\begin{frame}[fragile]\frametitle{}
\begin{center}
{\Large Production \texttt{.claude/} Setup}
\end{center}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{The Problem: Is Every Session Is Day One?}
  \begin{itemize}
    \item Most engineers use Claude Code like ChatGPT, every conversation is stateless, every session a blank slate.
    \item For real projects with a framework, conventions, deployment pipelines, and team standards, you're \textbf{throwing away context} every time you close the terminal.
    \item Symptoms:
      \begin{itemize}
        \item Re-explaining the stack 10 times a week.
        \item Claude generating \texttt{print()} instead of structured logging.
        \item Manually approving every \texttt{git diff} format.
        \item No guardrails against destructive commands.
      \end{itemize}
    \item The \texttt{.claude/} folder fixes this. It is a \textbf{persistent configuration layer} that loads automatically every session, your AI assistant's onboarding document that it actually reads and follows.
  \end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Demo: Add Path-Scoped Rules}

\begin{lstlisting}

## Step 1: Create the Python style rule:
...
Step 2: Verify it activates automatically:

# Edit any .py file and ask Claude to add a function.
# Observe: Claude enforces type hints and logger without
# being told -- the rule fired on the file glob match.

## Step 3: Add a security rule for all files:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Demo: Deny Rules in \texttt{settings.json}}

\textbf{Update \texttt{.claude/settings.json} with deny rules:}
\begin{lstlisting}
{
  "permissions": {
    "allow": [
...
# Ask Claude: "Show me the contents of .env"
# Observe: Claude cannot read it -- permission denied.

# Ask Claude: "Force push to main"
# Observe: Blocked by deny rule -- no hesitation needed.
\end{lstlisting}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Demo: Model-Optimized Subagents}


\textbf{Test:} Ask ``How does the chatbot handle conversation state?'', Haiku fires. Ask ``Review src/mychatbot/chain.py for security issues'', Sonnet fires.

\begin{lstlisting}
## Create the codebase explorer on Haiku (cheap + fast):
cat > .claude/agents/explore-codebase.md << 'EOF'
---
name: explore-codebase
description: >
  Answers "how does X work?" questions. Use when tracing
  request flows, finding where logic lives, or mapping
  module dependencies. DO NOT use for code changes.
model: haiku
tools: Read, Grep, Glob
---
You are a codebase explorer for this project.
Trace full request flows. Reference specific files and
line numbers. Keep answers concise with code references.
EOF

## Create the security-focused code reviewer on Sonnet:
cat > .claude/agents/code-reviewer.md << 'EOF'
---
name: code-reviewer
description: >
  Reviews code for bugs, security issues, and pattern
  compliance. Returns file:line references with fixes.
model: sonnet
tools: Read, Grep, Glob, Bash
---
Flag actual bugs and logic errors, not style nitpicks.
Check for security issues. Suggest specific fixes.
EOF
\end{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Demo: Auto-Triggered Security Skill}



\textbf{Test:} Edit \texttt{src/mychatbot/config.py}. Observe Claude automatically runs the security checklist, no \texttt{/review} needed.

\begin{lstlisting}

## Create a skill that fires automatically on API/config changes:
...
description: >
  Auto-triggered security checklist. Activates when
  editing API endpoints, config files, or Dockerfiles.
triggers:
  - "**/*router*.py"
  - "**/*config*.py"
  - "**/Dockerfile*"
...
Run through these checks automatically on every match:
1. No hardcoded secrets or tokens in source
2. All external inputs validated with Pydantic
3. No sensitive data appears in log statements
4. Docker containers run as non-root user
5. Dependency versions pinned (no bare `>=`)
...
\begin{frame}[fragile]\frametitle{}
\begin{center}
{\Large Stage 14: Deployment to Vercel}
\end{center}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Why Vercel for Deployment?}
\textbf{Vercel} provides seamless deployment for modern web applications with:
  \begin{itemize}
    \item \textbf{Zero-config deployment} from GitHub repositories
    \item \textbf{Automatic HTTPS} and CDN distribution
    \item \textbf{Continuous deployment} -- every git push triggers a new build
    \item \textbf{Preview deployments} for pull requests
    \item \textbf{Serverless functions} for backend API routes
    \item \textbf{Free tier} sufficient for most demo and small production apps
  \end{itemize}

\textbf{Perfect for our chatbot project:}
  \begin{itemize}
    \item Streamlit app runs in serverless environment
    \item Environment variables kept secure (API keys not in code)
    \item Live URL to share with anyone
    \item Auto-scales based on traffic
  \end{itemize}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Pre-Deployment Checklist}
Before deploying to Vercel, ensure you have:

\textbf{Code Ready:}
  \begin{itemize}
    \item All code committed to Git: \lstinline|git status| shows clean
    \item Tests passing: \lstinline|/test-run| completes successfully
    \item \texttt{requirements.txt} or \texttt{package.json} present with all dependencies
    \item \texttt{README.md} documentation complete
  \end{itemize}

\textbf{Accounts Set Up:}
  \begin{itemize}
    \item GitHub account created and logged in
    \item Vercel account created with \textbf{same email} as GitHub
    \item Repository pushed to GitHub: \lstinline|git remote -v| shows GitHub URL
  \end{itemize}

\textbf{Environment Configuration:}
  \begin{itemize}
    \item API keys not hardcoded in source files
    \item App designed to accept API keys via environment variables or UI input
    \item \texttt{.env} files listed in \texttt{.gitignore} (never commit secrets!)
  \end{itemize}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Step 1: Push Your Code to GitHub}
\textbf{Create a new repository and push your code:}

\begin{lstlisting}
# If not already initialized:
git init
git add .
git commit -m "Initial commit: chatbot ready for deployment"

# Create repo on GitHub (via web or CLI):
gh repo create mychatbot --public --source=. --remote=origin

# Or add remote manually:
git remote add origin https://github.com/YOUR_USERNAME/mychatbot.git
...
# Verify:
git remote -v
# Should show: origin https://github.com/YOUR_USERNAME/mychatbot.git
\end{lstlisting}

\textbf{Pro Tip:} Use descriptive commit messages. Vercel deployment logs will reference them.
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Step 2: Connect Vercel to GitHub}
\textbf{Authorize Vercel to access your GitHub repositories:}

  \begin{enumerate}
    \item Go to \url{https://vercel.com/dashboard}
    \item Click \textbf{``Add New...''} $\to$ \textbf{``Project''}
    \item Click \textbf{``Continue with GitHub''} (if not already connected)
    \item GitHub will ask for permissions:
      \begin{itemize}
        \item Grant Vercel access to repositories
        \item You can limit to specific repos or allow all
      \end{itemize}
    \item Return to Vercel dashboard
  \end{enumerate}

\textbf{What this does:}
  \begin{itemize}
    \item Enables Vercel to clone your repo
    \item Sets up webhooks for automatic deployments on push
    \item Allows preview deployments for pull requests
  \end{itemize}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Step 3: Import and Configure Your Project}
\textbf{Import your repository into Vercel:}

  \begin{enumerate}
    \item Vercel shows list of your GitHub repositories
    \item Find and click your repository (e.g., \texttt{mychatbot})
    \item Click \textbf{``Import''}
    \item Configure project settings:
      \begin{itemize}
        \item \textbf{Project Name:} Auto-filled from repo name (editable)
        \item \textbf{Framework Preset:} Vercel auto-detects (e.g., ``Other'' for Streamlit)
        \item \textbf{Root Directory:} Leave as \texttt{./} unless monorepo
        \item \textbf{Build Command:} Usually auto-detected or leave empty
        \item \textbf{Output Directory:} Usually auto-detected
      \end{itemize}
  \end{enumerate}

\textbf{Environment Variables (if needed):}
  \begin{itemize}
    \item For our chatbot: \textbf{leave empty} (users provide API keys in UI)
    \item For apps needing secrets: Add them here (e.g., \texttt{GROQ\_API\_KEY})
  \end{itemize}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Step 4: Deploy!}
\textbf{Trigger the deployment:}

  \begin{enumerate}
    \item Click \textbf{``Deploy''} button
    \item Vercel starts building:
      \begin{itemize}
        \item Clones repository
        \item Installs dependencies from \texttt{requirements.txt} / \texttt{package.json}
        \item Runs build commands
        \item Deploys to global CDN
      \end{itemize}
    \item Wait 2--5 minutes (watch build logs in real-time)
    \item You'll see \textbf{``Congratulations! Your site is live''}
  \end{enumerate}

\textbf{Your app is now live at:}
\begin{center}
\texttt{https://YOUR-PROJECT-NAME.vercel.app}
\end{center}

\textbf{Example:}
\begin{lstlisting}
https://mychatbot-abc123.vercel.app
\end{lstlisting}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Step 5: Test Your Live Application}
\textbf{Verify deployment success:}

  \begin{enumerate}
    \item Click the deployment URL from Vercel dashboard
    \item Test all features:
      \begin{itemize}
        \item For CLI chatbot: Functionality might need adaptation for web
        \item For Streamlit app: Test chat interface, conversation history, API key input
      \end{itemize}
    \item Check for errors in Vercel deployment logs if something breaks
    \item Test on mobile and desktop browsers
  \end{enumerate}

\textbf{Share your app:}
\begin{lstlisting}
# Your live URL:
https://mychatbot-abc123.vercel.app

# Share on social media, with friends, in your portfolio!
# Example: "Built a Groq-powered chatbot with Claude Code.
#          Try it live: https://mychatbot-abc123.vercel.app"
\end{lstlisting}

?? \textbf{Congratulations! Your application is live on the internet!}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Continuous Deployment in Action}
\textbf{Every git push now triggers automatic deployment:}
...
\textbf{Preview Deployments for Pull Requests:}
  \begin{itemize}
    \item Create a branch: \lstinline|git checkout -b feature/new-ui|
    \item Push changes: \lstinline|git push origin feature/new-ui|
    \item Open PR on GitHub
    \item Vercel creates a \textbf{preview URL} unique to that PR
    \item Test changes before merging to main!
  \end{itemize}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Managing Your Deployment}
\textbf{Vercel Dashboard Features:}
  \begin{itemize}
    \item \textbf{Deployments:} View all builds, rollback to previous versions
    \item \textbf{Analytics:} Track visitors, page views, performance
    \item \textbf{Logs:} Real-time function logs for debugging
    \item \textbf{Settings:} Custom domains, environment variables, integrations
  \end{itemize}

\textbf{Common Post-Deployment Tasks:}

\begin{lstlisting}
# Add a custom domain:
# Vercel Dashboard > Settings > Domains > Add
# Example: mychatbot.com -> mychatbot-abc123.vercel.app

# Update environment variables:
# Vercel Dashboard > Settings > Environment Variables
# Add/edit variables, then redeploy
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Troubleshooting Deployment Issues}
\textbf{Build Fails:}
  \begin{itemize}
    \item Check build logs in Vercel dashboard
    \item Common causes: Missing dependencies, wrong Python/Node version
    \item Solution: Update \texttt{requirements.txt}, add \texttt{runtime.txt} for Python version
  \end{itemize}

\textbf{App Crashes at Runtime:}
  \begin{itemize}
    \item Check Function logs in Vercel dashboard
    \item Common causes: Missing environment variables, API rate limits
    \item Solution: Add env vars, handle errors gracefully
  \end{itemize}

\textbf{Deployment Timeout:}
  \begin{itemize}
    \item Vercel free tier has 10-second execution limit for functions
    \item Streamlit apps might need optimization
    \item Solution: Reduce dependencies, optimize cold start time
  \end{itemize}
...
\textbf{Claude Code config committed to repo:}
  \begin{itemize}
    ...
    \item \texttt{.claude/rules/}: code-style, agent-patterns, security rules (path-scoped)
    \item \texttt{.claude/agents/}: specs, debug, docs, review, explore-codebase subagents
...
\textbf{Deployed to Production:}
  \begin{itemize}
    \item Live application hosted on Vercel at \texttt{https://YOUR-APP.vercel.app}
    \item Continuous deployment from GitHub (every push = new build)
    \item Shareable URL for portfolio and demonstrations
  \end{itemize}
\end{frame}
...
Deploy to Vercel & GitHub push + Vercel integration \\
Enforce code style auto & Path-scoped rule (\texttt{.claude/rules/}) \\
Block destructive cmds & Deny rules (\texttt{settings.json}) \\
Auto security review & Auto-triggered skill \\
...
\begin{frame}[fragile]\frametitle{Key Lessons from the Workshop}
  \begin{itemize}
    ...
    \item \textbf{Deny rules build trust.} Pre-authorizing operations is only half the picture. Deny \texttt{rm -rf}, \texttt{git push --force}, and \texttt{.env} reads so you never hesitate before approving an action.
    \item \textbf{Rules enforce themselves.} Path-scoped rules in \texttt{.claude/rules/} activate on file globs, no manual invocation. Edit a \texttt{.py} file and get PEP 8 enforcement; touch an agent file and get domain-specific patterns on top.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Keyboard Shortcuts Quick Reference}
\begin{center}
\adjustbox{max width=\linewidth}{%
\begin{tabular}{|l|l|}
\hline
...
\hline
\texttt{Shift+Tab} (once) & Toggle Plan mode (think before editing) \\
\texttt{Shift+Tab} (twice) & Toggle Auto-accept mode (bulk edits) \\
\texttt{Alt+T} & Toggle extended thinking (Opus only) \\
\texttt{Ctrl+C} & Interrupt Claude mid-response \\
\texttt{Ctrl+D} & Exit the session \\
\texttt{Ctrl+O} & Toggle verbose/debug mode (hook stdout visible) \\
\texttt{Ctrl+R} & Fuzzy-search command history \\
\texttt{$\uparrow$ / $\downarrow$} & Navigate previous prompts \\
\hline
\end{tabular}
}
\end{center}

\textbf{Slash commands to know:}
  \begin{itemize}
    \item \texttt{/init}: generate CLAUDE.md for current repo
    \item \texttt{/compact}: summarise context to save tokens
    \item \texttt{/cost}: show token usage and cost so far
    \item \texttt{/hooks}: list all active hooks
    \item \texttt{/model <id>}: switch model mid-session
...
\begin{frame}[fragile]\frametitle{Resources}
  \begin{itemize}
    \item \textbf{Claude Code Docs:} https://code.claude.com/docs
    \item \textbf{Claude Code Overview:} docs.claude.com/en/docs/claude-code/overview
    \item \textbf{Official GitHub:} github.com/anthropics/claude-code
    \item \textbf{Awesome Claude Code (community):} github.com/hesreallyhim/awesome-claude-code
    \item \textbf{Claude Code How-To Guide:} github.com/luongnv89/claude-howto
    \item \textbf{MCP Server Registry:} github.com/modelcontextprotocol/servers
    \item \textbf{LangChain + Groq:} python.langchain.com/docs/integrations/chat/groq
    \item \textbf{Groq Console (API keys):} console.groq.com
    \item \textbf{Streamlit Docs:} docs.streamlit.io
...
\begin{center}
\textit{Workshop is complete. You now have a production-ready Claude Code workflow.}\\[0.5em]
\textbf{Happy coding!}
\end{center}
\end{frame}