...
Bayes' theorem is just a recipe for updating a belief once new evidence arrives. You start with how rare the disease is ($P(D)$), fold in how reliable the test is, and get back a corrected probability that accounts for the actual test result -- not just gut feeling.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Bayes' Theorem}
\begin{block}{Think About It}
In the disease-test example, only 9.4\% of people with a positive test actually have the disease, even though the test is 90\% accurate on sick people. Why does such an accurate test still produce mostly false positives here?
\end{block}
\pause
\begin{block}{Answer}
Because the disease is rare (0.8\%) -- there are far more healthy people than sick people, so even a small 7\% false-positive rate among the huge healthy population produces more false alarms than true positives from the tiny sick population. This is the classic "base rate" trap.
\end{block}
\end{frame}
