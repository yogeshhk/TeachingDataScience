...
ANOVA asks: is the spread BETWEEN group means large compared to the spread WITHIN each group? If the groups' internal noise is small but their averages are far apart, that's a real difference. If the internal noise is huge, apparent gaps between averages could just be random scatter.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: ANOVA}
\begin{block}{Think About It}
You want to compare average test scores across 5 different teaching methods. Why is running one ANOVA better than running a separate t-test for every pair of methods?
\end{block}
\pause
\begin{block}{Answer}
Running many pairwise t-tests inflates the overall error rate -- each test carries its own 5\% chance of a false positive, so testing many pairs compounds that risk well above 5\% (as seen earlier, 5\% can balloon to 14.3\% with just 3 groups). ANOVA tests all groups at once with a single error rate, avoiding that inflation.
\end{block}
\end{frame}
