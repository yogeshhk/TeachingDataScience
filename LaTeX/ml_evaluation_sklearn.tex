...
\begin{frame}[fragile]\frametitle{Area Under ROC Curve}
	...
	\item The AUC represents a model’s ability to discriminate between positive and negative classes. An area of 1.0 represents a model that made all predictions perfectly. An area of 0.5 represents a model as good as random.
...
AUC = 0.5 isn't ``50\% accurate'' -- it means the model's ranking of positive vs. negative cases is no better than a coin flip. AUC is threshold-free: it asks ``if I pick one random positive and one random negative example, how often does the model score the positive one higher?'' -- which is why it stays meaningful even when the two classes are wildly imbalanced, unlike plain accuracy.
\end{block}
...
\begin{frame}[fragile]\frametitle{$R^2$ Metric}
	...
	\item $R^2 = 1$ is a perfect fit and $R^2 = 0$ means the model does no better than always predicting the mean -- but $R^2$ is \emph{not} bounded below by 0: a model that fits worse than that mean-only baseline (common on a bad fold) can score negative, which is exactly why the example below has such a large standard deviation relative to its mean.
	\end{itemize}
...
already better, so this mean can be read directly. The large standard deviation relative to the
mean here is exactly the negative-$R^2$-on-a-bad-fold effect described on the previous slide.
\begin{lstlisting}
seed = 7
kfold = model_selection.KFold(n_splits=10, shuffle=True, random_state=seed)
model = LinearRegression()
