...
Think of a smoke alarm. Type I error is a false alarm -- it goes off when there's no fire (you rejected $H_0$ wrongly). Type II error is staying silent during an actual fire -- it fails to warn you when it should (you failed to reject $H_0$ wrongly). Tuning the alarm to reduce one type of error usually increases the other.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Hypothesis Testing}
\begin{block}{Think About It}
A researcher says "my test proved the null hypothesis is true." What's wrong with that statement, based on what a hypothesis test can actually tell you?
\end{block}
\pause
\begin{block}{Answer}
A hypothesis test can never prove $H_0$ is true -- it can only fail to reject it (not enough evidence against it) or reject it (enough evidence against it). "Failing to reject" is not the same as "proving true."
\end{block}
\end{frame}
