...
\begin{lstlisting}
# In Rasa 2.x, dialog training is done via CLI:
#   rasa train
# Policy config goes in config.yml, e.g.:
#   policies:
...
\begin{lstlisting}
# Loading the Agent (Rasa 2.x: interpreter is built-in when loading from a full model)
...
online\_train.py:
\begin{lstlisting}
# In Rasa 2.x, interactive/online training uses the CLI:
#   rasa interactive
# This replaces the deprecated train_online() / ConsoleInputChannel / RasaNLUInterpreter approach.
# The CLI reads domain.yml, config.yml, data/ automatically and
# lets you correct bot predictions interactively to build new stories.
\end{lstlisting}
