...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: The Scraping Trio}
\begin{block}{Intuition}
Think of it like receiving mail: \textbf{Requests} is the courier who fetches the sealed
envelope (the raw HTML) from the web server; \textbf{Beautiful Soup} is you opening the
envelope and organizing the letter's contents so you can find what you need; \textbf{Selenium}
is a robot that impersonates you, actually clicking buttons and filling forms in a real browser
window, for pages that won't hand over their content any other way.
\end{block}
\end{frame}
...
\end{lstlisting}
\item  We can also iterate through them. Note that \texttt{.string} produces a NavigableString
  object while \texttt{.text} is typical unicode text -- and if a paragraph has child tags,
  \texttt{.string} can't pick one unambiguously and returns \texttt{None}:
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Data Mining}
\begin{block}{Think About It}
You use \lstinline|.string| on a \texttt{<p>} tag that contains a nested \texttt{<b>} tag, and get
\texttt{None} back instead of the text you expected. What's happening, and what should you use
instead?
\end{block}
\pause
\begin{block}{Answer}
\lstinline|.string| only works when a tag has exactly one child that is itself plain text -- if
the tag contains nested tags (like \texttt{<b>}), Beautiful Soup can't unambiguously pick ``the''
text and returns \texttt{None}. Use \lstinline|.text| (or \lstinline|.get_text()|) instead, which
concatenates all the text inside a tag regardless of nesting.
\end{block}
\end{frame}
