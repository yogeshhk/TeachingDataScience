...
\item We can fix this easily; see the resulting shape.
\end{itemize}
\begin{lstlisting}
...
\item Note: The step size is (1,1) by default, and it can be tuned using the 'subsample' parameter.
\end{itemize}
\begin{lstlisting}
model = Sequential()
...
\item You can also use a variety of callbacks to set early-stopping rules, save model weights along the way, or log the history of each training epoch.
\end{itemize}
\begin{lstlisting}
