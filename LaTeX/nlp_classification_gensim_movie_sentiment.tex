...
\end{itemize}
\begin{lstlisting}
model = Doc2Vec(min_count=1, window=10, vector_size=100, sample=1e-4, negative=5, workers=2)
...
for i in range(2): # 10 if good machine
    ...
    model.train(taggedDocs, total_examples=model.corpus_count, epochs=10)
\end{lstlisting}
\end{frame}
...
\item We can access all of them using model.wv.vectors (the word embedding matrix of the model).
\item However, we don't want to use the entire wv.vectors since that contains the vectors for the words as well, but we are only interested in the ones for sentences (accessed via model.dv).
\end{itemize}

Here's a sample vector for the first sentence in the training set for negative reviews:
\begin{lstlisting}
model.dv['TRAIN_NEG_0']
...
for i in range(12500):
    ...
    train_arrays[i] = model.dv[prefix_train_pos]
    train_arrays[12500 + i] = model.dv[prefix_train_neg]
    train_labels[i] = 1
