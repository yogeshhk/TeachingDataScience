...
\item Around 99.7\% of a normal sample lies within three standard
  deviations of the mean.  Continuing with the previous example,
  around 997 of the points will lie between 4 and 16.

\end{itemize}

\begin{block}{Intuition}
This is the classic 68-95-99.7 rule. Each extra standard deviation you allow captures almost all of the remaining outliers -- which is why going from 2 to 3 standard deviations only adds a small sliver more (95\% to 99.7\%), even though it sounds like a big jump in "distance."
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Normal Distribution}
\begin{block}{Think About It}
Why do we usually log-transform income data before analyzing it, but not age data?
\end{block}
\pause
\begin{block}{Answer}
Income is typically strongly right-skewed (a few very high earners stretch the tail), and log-transforming symmetrizes that skew so it behaves more like a normal distribution. Age is usually already roughly symmetric, so there's nothing to fix -- transforming it would just make interpretation harder for no benefit.
\end{block}
\end{frame}
