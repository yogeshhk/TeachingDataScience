...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Prepare Yourself to Competitions!: My Solution}
Assigning 3 people to 3 roles has exactly $3! = 6$ possible pairings -- rather than hand-writing each one (error-prone as the team grows), \texttt{itertools.permutations} generates every role assignment for us.
\begin{lstlisting}
...
# Each permutation of (0,1,2) assigns a role-column to each person-row
c = max(getDistance(m[0][roles[0]], m[1][roles[1]], m[2][roles[2]])
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Competitive Programming Setup}
\begin{block}{Think About It}
Every solution in this section reads from \texttt{input.txt} and writes to \texttt{output.txt} instead of using \texttt{input()}/\texttt{print()}. Why do competitive-programming judges typically prefer file I/O over interactive console I/O?
\end{block}
\pause
\begin{block}{Answer}
File I/O is deterministic and automatable: a judge can feed thousands of test cases to your program without a human typing anything, redirect output to compare it byte-for-byte against an expected answer file, and run everything in a sandboxed batch job with no terminal attached. \texttt{input()} assumes an interactive terminal is present, which judges don't provide.
\end{block}
\end{frame}
