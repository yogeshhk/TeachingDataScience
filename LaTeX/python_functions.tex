...
\begin{frame}[fragile]\frametitle{The `return' statement }
    ...
    \item If there is no return statement in the function code, the function ends, when the control flow reaches the end of the function body and the value \texttt{None} will be returned.
    \end{itemize}
\end{frame}
...
\end{lstlisting}
    ...
    \item The body of f() consists solely of the \texttt{print(s)} statement.
...
\begin{frame}[fragile]\frametitle{Variable Scope}
    ...
    \item A variable can't be both local and global inside of a function. 
    	...
    	\item To tell Python, that we want to use the global variable, we have to explicitly state this by using the keyword \texttt{global}
    \end{itemize}
...
Think of the argument as a sticky note with an address on it, not the object itself: the function gets its own copy of the sticky note (so reassigning the note doesn't affect the caller's note), but both notes point to the same object -- so if that object is mutable, editing it \emph{through} the note is visible to everyone holding a note to it.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Functions}
\begin{block}{Think About It}
A function that receives a list and calls \texttt{mylist.append(x)} changes the caller's list. A function that receives an integer and does \texttt{n = n + 1} does not change the caller's integer. Both arguments were passed the same way -- so why the different outcome?
\end{block}
\pause
\begin{block}{Answer}
Python always passes an object reference (the ``sticky note'' pointing to the object), never a copy of the value. \texttt{mylist.append(x)} mutates the object the reference points to -- everyone holding a reference to that list sees the change. \texttt{n = n + 1} doesn't mutate anything; it creates a brand-new integer object and rebinds the local name \texttt{n} to point at it, leaving the caller's \texttt{n} pointing at the original object. What matters isn't how the argument was passed -- it's whether the object itself is mutable.
\end{block}
\end{frame}