...
We assumed $\sqrt{2}$ COULD be written as a fraction in lowest terms, then showed that forces both the numerator and denominator to be even -- meaning it wasn't actually in lowest terms after all. That self-contradiction is what proves no such fraction can exist.
\end{block}
...
\begin{block}{Intuition}
Induction is like knocking over dominoes: showing the base case is knocking over domino 0, and the induction step is showing "if any domino falls, so does the next one." Together, that guarantees every domino falls -- no matter how many there are.
\end{block}
...
\begin{block}{Intuition}
Injective: no two different inputs ever collide on the same output. Surjective: every possible output actually gets hit by something. Bijective: both at once, so every input pairs with exactly one output and vice versa -- a perfect one-to-one matching.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Proofs \& Foundations}
\begin{block}{Think About It}
To prove two infinite sets are "the same size," we look for a bijection between them. Why does simply finding an injective function (no bijection needed) NOT prove they're the same size?
\end{block}
\pause
\begin{block}{Answer}
An injective function only guarantees the target set is at least as big -- it doesn't rule out the target having extra unmatched elements. Only a bijection (matching in both directions) rules out one set being strictly bigger than the other.
\end{block}
\end{frame}
