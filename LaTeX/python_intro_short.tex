...
% Duplicate "Why Python?" frame removed (same content as the frame above)
...
\begin{itemize}
\item Python 2.7 reached \textbf{end-of-life on January 1, 2020} --- do not use it for new code.
\item \textbf{Python 3} (current stable: 3.12+) is the only supported version.
\item This workshop uses \textbf{Python 3} throughout.
\end{itemize}
\end{frame}
...
\pause
\begin{block}{Answer}
It's a property of the \emph{implementation}, not the language. Python the language has no built-in notion of how it must be run -- CPython interprets it, but PyPy JIT-compiles it, and tools like Cython compile it to machine code. ``Python is interpreted'' really means ``the standard CPython tool interprets it,'' not that the language itself demands interpretation.
\end{block}
\end{frame}
