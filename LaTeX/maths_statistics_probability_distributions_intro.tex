...
\begin{itemize}
\item function that describes continuous random variables with probabilities
\item $P(a < X < b) = \int_a^b f(x)\,dx$ is the probability that $X$ falls in the range $(a,b)$ -- the AREA under the curve, not the height at a point.
\item Properties:
\begin{itemize}
\item $f(x) \geq 0$ (the density itself can exceed 1 -- only the total area is bounded)
\item $\int_{-\infty}^{\infty} f(x)\,dx = 1$
\end{itemize}
\end{itemize}

\begin{block}{Intuition}
A common trap: $f(x)$ for a continuous variable is NOT a probability, and $P(X=x)$ is always exactly 0 for any single point (there are infinitely many points to spread probability across). Only areas under the curve -- over a range, not a point -- give you real probabilities.
\end{block}
\end{frame}
