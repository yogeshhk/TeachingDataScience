...
This whole list is just saying vector addition and scalar multiplication behave exactly like ordinary number arithmetic -- order doesn't matter, grouping doesn't matter, there's a "do nothing" zero and a "do nothing" scalar (1). None of it is new; it's confirming vectors don't misbehave.
\end{block}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Vectors}
\begin{block}{Think About It}
Two vectors have the same magnitude but point in opposite directions. Is their sum the zero vector, or something else?
\end{block}
\pause
\begin{block}{Answer}
The zero vector. If $\vec{v}$ and $-\vec{v}$ have equal magnitude and opposite direction, then $\vec{v} + (-\vec{v}) = \vec{0}$ -- they exactly cancel, which is precisely what "additive inverse" means.
\end{block}
\end{frame}
