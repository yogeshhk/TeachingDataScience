...
for i in range(6):
    print(i**2)
\end{lstlisting}
\texttt{range()} in Python 3 is \textbf{lazy} --- it produces values one at a time without building the full list in memory. The old Python 2 \texttt{xrange()} no longer exists; \texttt{range} is its direct replacement.
