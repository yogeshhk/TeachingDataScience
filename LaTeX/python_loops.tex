...
\begin{frame}[fragile]
  \frametitle{\texttt{for}-loops}
    ...
    a sequence. To break out of a \texttt{for} loop, use the \texttt{break}
    statement; to jump to the next iteration, use the \texttt{continue} statement.
\begin{lstlisting}
for i in range(0, 4):
  # loop block
  print (i*i)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Looping over a range of numbers}
\tiny{(Ref: Transforming Code into Beautiful, Idiomatic Python -  Raymond Hettinger)}

Two ways (prefer \texttt{range}). \texttt{range()} in Python 3 is \textbf{lazy} --- it produces values one at a time without building the full list in memory. The old Python 2 \texttt{xrange()} no longer exists; \texttt{range} is its direct replacement.
\begin{lstlisting}
...
\begin{lstlisting}
>>> for val in 'UZH':   # iterating over a string gives characters
...   print(val)
U
Z
H
...
``C'' way. \texttt{enumerate} is fast and beautiful, and saves you from tracking indices and incrementing them by hand -- whenever you find yourself manipulating indices in a collection, you're probably doing it wrong.
\begin{lstlisting}
colors = ['red', 'green', 'blue', 'yellow']
...
\texttt{zip} works like a zipper: it pairs up the $i$-th item of each collection, stopping as soon as the shorter one runs out. In Python 2, \texttt{zip()} eagerly built a whole list in memory (the lazy version was called \texttt{izip}); in Python 3, \texttt{zip()} itself is lazy, producing pairs one at a time.
\end{block}
\begin{lstlisting}
...
Write the program to produce output like this. Use the \texttt{random} module to get the secret number.
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Loops}
\begin{block}{Think About It}
A \texttt{while} loop's \texttt{else} clause runs when the loop finishes normally, but is skipped if the loop exits via \texttt{break}. What real-world task is this \texttt{while}/\texttt{else} pattern a natural fit for?
\end{block}
\pause
\begin{block}{Answer}
Searching for something and reacting differently to ``found'' vs ``not found'': loop through a list looking for a match and \texttt{break} when you find it; if the loop completes without breaking (nothing matched), the \texttt{else} clause runs your ``not found'' logic. It replaces a separate \texttt{found = False} flag variable you'd otherwise have to track by hand.
\end{block}
\end{frame}
