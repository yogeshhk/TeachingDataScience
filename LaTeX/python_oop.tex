...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: Classes vs.\ Objects}
\begin{block}{Intuition}
Think of a \emph{class} as a blueprint (e.g., for a house) and an \emph{object} as one actual
house built from that blueprint. Many houses (objects) can be built from the same blueprint
(class), each with its own address, paint color, and furniture (instance attributes) -- but all
sharing the same floor plan (methods).
\end{block}
\end{frame}
...
Python provides us with many built-in objects. Below, the first line constructs an object of
type \texttt{list}; the second and third lines call the \lstinline|append()| method; the fourth
line calls \lstinline|sort()|; the fifth line retrieves the item at position 0; and the sixth
line calls \lstinline|__getitem__()| on the list directly, with a parameter of zero:
\begin{lstlisting}
stuff = list()
stuff.append('python')
stuff.append('chuck')
...
Import the \lstinline{date} class from the standard library module \lstinline{datetime}. To instantiate an object, call the class name like a function:
\begin{lstlisting}
...
When the party method is called, the first parameter (\texttt{self}) points to the particular
instance of the \texttt{PartyAnimal} object that \texttt{party} is called from within -- note the
line \lstinline|self.x = self.x + 1| inside the method:
\begin{lstlisting}
class PartyAnimal:
	x = 0
	def party(self) :
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: Why \texttt{self}?}
\begin{block}{Intuition}
Read \texttt{self.x} as ``\emph{my} x'' -- it tells Python which object's data to use. Without
\texttt{self}, a method would have no way to know \emph{which} instance called it, since the
same method body is shared by every instance of the class.
\end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Member functions}
There are different types of methods/functions -- instance, class, and static -- shown below.
(In Python 3, all classes are ``new-style'' by default; explicitly inheriting from
\texttt{object}, e.g. \texttt{class MyClass(object):}, is optional but still common.)
\begin{lstlisting}
class MyClass:
    def method(self):
...
Through the \texttt{self} parameter, instance methods can freely access attributes and other
...
Marked with a \texttt{@classmethod} decorator, a class method takes a \texttt{cls} parameter
that points to the class itself -- not the object instance -- when called:
  \begin{itemize}
  \item Because it only has access to \texttt{cls}, a class method can't modify object instance state -- that would require \texttt{self}.
  \item However, class methods can still modify class state that applies across all instances of the class.
  \end{itemize}
\begin{lstlisting}
class MyClass:
    @classmethod
...
Marked with a \texttt{@staticmethod} decorator, this type of method takes neither a \texttt{self}
nor a \texttt{cls} parameter (though it's free to accept other parameters):
  \begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: Instance vs.\ Class vs.\ Static Methods}
\begin{block}{Intuition}
Picture a workshop: an \emph{instance method} works on one specific project on the bench
(\texttt{self}); a \emph{class method} works on the workshop's shared rulebook (\texttt{cls}),
affecting every project; a \emph{static method} is just a tool kept in the workshop's toolbox --
useful and related to the workshop, but it doesn't touch any specific project or the rulebook at
all.
\end{block}
\end{frame}
...
Calling \lstinline|obj.method()| confirms that the instance method has access to the object
instance (printed as ``MyClass object'') via the \texttt{self} argument. When the method is
called, Python replaces the \texttt{self} argument with the instance object, \texttt{obj}:
\begin{lstlisting}
>>> obj = MyClass()
>>> obj.method()
('instance method called', <__main__.MyClass object at 0x101a2f4c8>)
\end{lstlisting}
...
\begin{frame}[fragile]\frametitle{Member functions: Class Methods}
  ...
  \begin{itemize}
  \item Calling \lstinline|classmethod()| shows it doesn't have access to a ``MyClass object'' instance, but only to the class \texttt{MyClass} itself, representing the class as an object (everything in Python is an object, even classes themselves).

  \item Naming these parameters \texttt{self} and \texttt{cls} is just a convention -- you could just as easily name them \texttt{the\_object} and \texttt{the\_class} and get the same result.
  \end{itemize}
\begin{lstlisting}
>>> obj.classmethod()
...
\begin{frame}[fragile]\frametitle{Member functions: Static Methods}
  ...
  \begin{itemize}
  \item It may be surprising that a static method can be called on an object instance at all -- behind the scenes, Python simply skips passing \texttt{self} or \texttt{cls} when a static method is invoked via dot syntax.
  \item This confirms that static methods can neither access the object instance state nor the class state -- they work like regular functions that happen to live in the class's (and every instance's) namespace.
  \end{itemize}
\begin{lstlisting}
>>> obj.staticmethod()
...
When we attempt to call these methods on the class itself - without creating an object instance
beforehand, \texttt{classmethod()} and \texttt{staticmethod()} still work, but \texttt{method()}
fails: there is no instance for Python to populate the \texttt{self} argument with.
\begin{lstlisting}
>>> MyClass.classmethod()
('class method called', <class '__main__.MyClass'>)
...
Rather than computing the area directly inside \lstinline|area()|, the well-known circle-area
formula is factored out into a separate \lstinline|circle_area()| static method:
\begin{lstlisting}
import math
...
Getters and setters, using the \texttt{@property} decorator. The instance variable is marked
private by prefixing it with an underscore; note that the instance variable and the property
must have different names, or we get infinite recursion:
\begin{lstlisting}
...
\begin{frame}[fragile]\frametitle{Member function: Example Construction Destruction}
   ...
   \begin{itemize}
  \item Using this class shows that \texttt{del} doesn't directly call the \_\_del\_\_() method.
  \item The destructor is not called when we delete \texttt{x1}: \texttt{del} only decrements the reference count for that object by one.
  \item Only once the reference count reaches zero -- here, after \texttt{x2} (the last remaining reference) is also deleted -- is the destructor actually called.
     \end{itemize}
\begin{lstlisting}
...
To override a method in the base class, sub class needs to define a method of same signature. (i.e same method name and same number of parameters as method in base class). Try commenting out \lstinline|m1()| in class \texttt{B} afterward -- the version from base class \texttt{A} will run instead:
...
Take a look at this Python interaction -- what do you expect \lstinline|print(u)| to show?
(Spoiler: not a \texttt{ComplexNum}.)
\begin{lstlisting}
>>> z = ComplexNum(1,0)
>>> print(z)
1 + i0
...
\begin{frame}[fragile]\frametitle{Polymorphism, II}
  ...
  integer number. Our code for \texttt{mul} implicitly assumes that
  argument \texttt{other} has attributes \texttt{x} and \texttt{y},
  and integers do not:
\begin{lstlisting}
>>> print (z * 3)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Modern Python: \texttt{@dataclass}}
Every class in this deck (\texttt{Car}, \texttt{Greeting}, \texttt{Vector}, \texttt{ComplexNum}, ...)
hand-wrote its own \texttt{\_\_init\_\_}. Since Python 3.7, the \texttt{@dataclass} decorator
generates \texttt{\_\_init\_\_}, \texttt{\_\_repr\_\_}, and \texttt{\_\_eq\_\_} automatically from
type-annotated fields -- this is purely a convenience for simple ``data-holding'' classes, and you
can still add your own methods exactly as before:
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Object-Oriented Programming}
\begin{block}{Think About It}
Two classes, \texttt{Vector} and \texttt{ComplexNum(Vector)}, both define \texttt{\_\_mul\_\_}
and \texttt{\_\_str\_\_}, but only \texttt{Vector} defines \texttt{\_\_add\_\_}. If \texttt{z}
and \texttt{w} are both \texttt{ComplexNum} instances, what type is \texttt{z + w}, and why does
that matter?
\end{block}
\pause
\begin{block}{Answer}
\texttt{z + w} is a plain \texttt{Vector}, not a \texttt{ComplexNum} -- because
\texttt{\_\_add\_\_} is inherited unchanged from \texttt{Vector}, and its body hard-codes
\texttt{return Vector(...)} rather than using \texttt{self.\_\_class\_\_(...)}. This is exactly
the bug seen in ``Inheritance, IV/V'': a method inherited by a subclass still constructs
instances of the \emph{class it was written in}, unless it's written to respect the actual
instance's class.
\end{block}
\end{frame}
