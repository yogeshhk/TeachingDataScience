...
No single ticket ever actually loses "\$19.33" -- you either lose \$20 or win \$980. Expected value is what you'd average per ticket if you (or everyone) played this raffle over and over; a negative expected value just means the game is, on average, a losing proposition for buyers (which is exactly how fundraisers make money).
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Expected Value \& Variance}
\begin{block}{Think About It}
Two investment options have the same expected return. Option A has a much higher variance than Option B. Which one is "riskier," and what does variance actually measure here?
\end{block}
\pause
\begin{block}{Answer}
Option A is riskier. Variance measures the expected squared spread of outcomes around the mean -- a higher variance means actual results are likely to swing further from the expected value in either direction, even though both options average out to the same return.
\end{block}
\end{frame}
