...
\texttt{max\_depth} is a bias-variance knob: too shallow (depth 1) and the tree can barely split the data at all -- high bias, underfitting. Too deep (unlimited) and it carves out a leaf for almost every training point -- high variance, overfitting, exactly like the fully-grown tree in the earlier conceptual section. The RMSE-vs-depth curve typically dips to a sweet spot in between, which is what this sweep is searching for.
\end{block}
...
def feature_importance_chart(clf, classifier_name, feature_names):
    # Sort features by Gini importance (ascending) for a readable bar chart
    sorted_feature_importances, sorted_feature_names = (
        zip(*sorted(zip(clf.feature_importances_, feature_names)))
    )
    plt.figure(figsize=(16, 9))
    # Horizontal bar chart: each bar = one feature's importance score
    plt.barh(range(len(sorted_feature_importances)), sorted_feature_importances)
    # Label each bar with feature name and its numeric importance
    plt.yticks(
...
# Plot importance for the trained simple tree
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Decision Trees with Scikit-Learn}
\begin{block}{Think About It}
The Car Evaluation example trains a \texttt{max\_depth=6} tree (Accuracy printed) and later a \texttt{max\_depth=None} ``fully perfect'' tree, comparing their \texttt{feature\_importances\_} charts. If the full-depth tree reaches close to 100\% accuracy on this same training data, why bother looking at the shallower depth-6 tree's feature importances at all?
\end{block}
\pause
\begin{block}{Answer}
Training accuracy alone doesn't tell you whether importances \emph{generalize} -- a fully-grown tree can reach near-100\% by carving out tiny leaves for individual quirky rows, and \texttt{feature\_importances\_} on that tree partly reflects those overfit splits, not just genuinely predictive structure. The shallower tree is forced to spend its limited splits on the features that separate the \emph{most} data, so its importance ranking is a cleaner signal of what actually matters -- the same bias-variance trade-off from \texttt{max\_depth} tuning, just viewed through feature importance instead of RMSE.
\end{block}
\end{frame}
