...
# getQgrams("Panos", 1) should return ["P", "a", "n", "o", "s"]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: q-grams \& Jaccard Similarity}
\begin{block}{Intuition}
Breaking each name into overlapping q-grams is like tearing it into a bag of small overlapping
Scrabble-tile chunks. Two names that are typos or naming variants of each other (``McDonald's''
vs ``McDonalds'') will share most of their tiles even though the full strings don't match
exactly -- so comparing the two bags of tiles (via the Jaccard coefficient) catches near-matches
that a strict string comparison would miss.
\end{block}
\end{frame}
...
\lstinline|getQgrams('Panos', 2)| returns the 2-grams of \texttt{"Panos"}. If two company names
share 3 out of 4 q-grams, is the Jaccard similarity \(3/4\)? Why or why not?
\end{block}
\pause
\begin{block}{Answer}
No -- Jaccard similarity is intersection over \emph{union}, not intersection over one side's
count. If name A has 4 q-grams and name B has a different (possibly overlapping) set, the
similarity is \(|A \cap B| / |A \cup B|\), not ``shared / one side's count''. This is exactly
the kind of corner case worth testing once STEP 3's \lstinline|computeSimilarity| function is
written.
\end{block}
\end{frame}
