...
\item  If A and C are independent, $P(A \cap C) = P(A)P(C)$.
%\item  If A and C are dependent, $P(A \cap C) = P(A)P(C)$.
\end{itemize}
\end{frame}
...
Conditioning on $A$ means "shrink the universe down to just the cases where $A$ happened." Within that smaller world, $P(B|A)$ asks what fraction now includes $B$ too -- that's exactly why it's $P(A \cap B)$ divided by $P(A)$, not the full sample space.
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Joint, Conditional \& Marginal Probability}
\begin{block}{Think About It}
If two events $A$ and $B$ are independent, what does that tell you about $P(A|B)$ compared to $P(A)$?
\end{block}
\pause
\begin{block}{Answer}
They're equal: $P(A|B) = P(A)$. Independence means knowing $B$ happened gives you no new information about $A$ -- so conditioning on $B$ shouldn't change $A$'s probability at all.
\end{block}
\end{frame}
