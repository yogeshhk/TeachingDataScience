...
\begin{block}{\scriptsize Intuition}
{\scriptsize O(1) is an \emph{average}, not a guarantee: a dict is really a fixed-size array where a hash function picks the slot, so a lookup is normally one jump straight to the answer -- degrading to O(n) only if many keys collide into the same slot.}
\end{block}
...
\begin{block}{\scriptsize Intuition}
{\scriptsize Memoization is just ``show your work so you never redo it'': the naive version below recomputes \texttt{fib(2)} dozens of times inside a single call, while caching each result the first time turns an exponential tree of calls into one pass per unique \texttt{n}.}
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: DSA \& Patterns}
...
\pause
\begin{block}{Answer}
Two Pointers relies on the sequence being \emph{indexable and ordered} (usually sorted) so that moving \texttt{left}/\texttt{right} inward is meaningful -- a linked list has no index to jump to, so you can't "look ahead." Fast \& Slow Pointers relies only on \texttt{.next} links and different speeds catching up to each other -- it needs no ordering or indexing, which is exactly why it works on cyclic structures where Two Pointers has no notion of "the other end."
\end{block}
\end{frame}