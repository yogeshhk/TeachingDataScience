...
Let's see how we can teach a simple conversation like that to Rasa. This is called a user story path. I have provided a few stories in the data/stories.md file for your reference. This is the training data for Rasa Core. The general format is:
...
Now, generate a similar graph for your stories using the following command. This is very helpful when debugging the conversational flow of the chatbot.
...
\item \lstinline|rasa run actions|
\end{itemize}
...
agent = Agent('domain.yml', policies = [MemoizationPolicy(max_history=2), TEDPolicy(epochs=200)])
...
\item Come to directory where actions.py is and then run
\item \lstinline|rasa run --endpoints endpoints.yml|
\end{itemize}
...
\lstinline|rasa run actions|


\begin{lstlisting}
import IPython
from IPython.display import clear_output
# Rasa 2.x: use rasa.core.* instead of rasa_core.*
from rasa.core.agent import Agent
...
def load_assistant():
    ...
    # Build NLU interpreter from the trained model directory
    interpreter = NaturalLanguageInterpreter.create(model_directory)
    # Point to the running action server (rasa run actions)
    endpoint = EndpointConfig('http://localhost:5055/webhook')
    # Load the trained dialogue model with the NLU interpreter and action endpoint
    ...
    while True:
        ...
        if a == 'stop':
            break
        # Send user text to the agent and print all response messages
