...
Think of \texttt{try}/\texttt{except}/\texttt{finally} as a safety net under a tightrope: \texttt{try} is the risky walk, \texttt{except} is what catches you if you fall (a specific kind of fall), and \texttt{finally} is the crew that cleans up the arena afterward -- whether you fell or not.
\end{block}
\begin{lstlisting}
try:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Exceptions}
\begin{block}{Think About It}
If you write \texttt{except Exception as ex:} and just \texttt{pass} inside, you've silenced every possible error path in that block. What's the danger, and when (if ever) is it a defensible choice?
\end{block}
\pause
\begin{block}{Answer}
The danger is masking real bugs -- a typo, a missing file, a network timeout, and a genuine logic error all get swallowed identically, so failures happen silently and are much harder to diagnose later. It's occasionally defensible for a truly optional, best-effort operation (e.g. an analytics ping that shouldn't crash the app if it fails) -- but even then, logging the exception rather than silently passing is almost always the safer choice.
\end{block}
\end{frame}
