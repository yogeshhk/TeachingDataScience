...
\begin{frame}[fragile]\frametitle{Variables}
  ...
  \begin{itemize}
  \item A variable can be seen as a label (or name tag) attached to a value.
  ...
  \item In Python, the \textbf{value} has a type --- the variable itself does not.
  \item The same variable name can point to an integer now and a string later.
    \end{itemize}
...
\begin{frame}[fragile]\frametitle{Basic Data types}
  ...
  \item {\bf int} Integer numbers, arbitrary precision (no separate \texttt{long} type in Python 3).
  \item{\bf float} Double precision floating-point numbers
  \item {\bf str} Unicode text (immutable sequence of characters).
...
\begin{frame}[fragile]\frametitle{Numbers}
  ...
  \item Unlimited precision integers : \lstinline{999999999999}
  \item Float : \lstinline{3.1415, 2.7122}
   \item Oct and hex :\lstinline|0o177, 0x9ff|
...
Can be indexed (subscripted), start at 0. No separate character type: a string of size one.
\begin{lstlisting}
word = 'Python'
...
Slice direction is always left to right.
\begin{block}{Intuition}
Think of index positions as marks \emph{between} characters, not the characters themselves: \texttt{word[a:b]} means ``everything from mark a up to (but not including) mark b.'' Negative indices count backwards from the end, so \texttt{word[-3:]} always means ``the last 3 characters,'' whatever the string's length.
\end{block}
\begin{lstlisting}
...
A copy of \texttt{s} is made, modified and returned.
  \begin{lstlisting}
...
\begin{frame}[fragile]\frametitle{Split}
    Split \texttt{s} at every occurrence of \texttt{t} and return a list of parts. If \texttt{t} is omitted, split on whitespace.
  \begin{lstlisting}
  	s.split(t)
  \end{lstlisting}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Finding text within string variables}
Exercise: Convert the code below to make it case-insensitive.
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Data Types}
\begin{block}{Think About It}
After \texttt{i = 42} then \texttt{i = "forty"}, has the variable \texttt{i} changed its type, or has something else happened?
\end{block}
\pause
\begin{block}{Answer}
Something else: \texttt{i} never had a type to begin with. Variables in Python are just names pointing to objects; \texttt{i = 42} makes the name \texttt{i} point to an integer object, and \texttt{i = "forty"} makes the same name point to a completely different string object. The old integer object is unchanged (and gets garbage-collected once nothing references it) -- only the \emph{name} moved. This is what ``dynamically typed'' means: types belong to values, not to variable names.
\end{block}
\end{frame}
