...
The sign of skewness points to where the LONG TAIL is, not where most of the data sits. A long tail stretching right (a few very large values) gives positive skew, even though most values pile up on the left.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Asymmetry \& Correlation}
\begin{block}{Think About It}
Two variables have a correlation of $R = -0.9$. Does that mean they're weakly related, strongly related, or unrelated? What does the negative sign tell you?
\end{block}
\pause
\begin{block}{Answer}
Strongly related -- $|R|$ close to 1 means a strong linear relationship, regardless of sign. The negative sign means the relationship is inverse: as one variable increases, the other tends to decrease.
\end{block}
\end{frame}