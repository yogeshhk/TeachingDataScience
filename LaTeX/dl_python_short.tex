...
The `.T' function, transposes the matrix from horizontal to vertical. So the computer is storing the vertical numbers horizontally!!.
\begin{lstlisting}
from numpy import exp, array, random, dot
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile] \frametitle{Constructing the Python code}
Iteration loop. Weights are settled now.
\begin{lstlisting}
for iteration in range(10000):
    output = 1 / (1 + exp(-(dot(training_set_inputs, synaptic_weights))))
    synaptic_weights += dot(training_set_inputs.T, (training_set_outputs - output) * output * (1 - output))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile] \frametitle{Constructing the Python code}
Prediction of unknown. That's it!!
