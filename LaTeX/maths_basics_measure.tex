...
\item $\mu(\emptyset) = 0$
\item If $A$ is subset of $B$ then $\mu(A) \leq \mu(B)$
...
These properties just say a measure behaves the way "size" should: nothing has negative size, a bigger set has at least as much size as any of its parts, and if you break a region into non-overlapping pieces, their sizes simply add up. Probability is just measure with the extra rule that the whole sample space has size exactly 1.
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Measure Theory}
\begin{block}{Think About It}
Why must a probability measure satisfy $P(\Omega) = 1$ specifically, rather than some other fixed number?
\end{block}
\pause
\begin{block}{Answer}
$\Omega$ is the set of \emph{all} possible outcomes -- something in it is guaranteed to happen. Probability is defined so "certainty" corresponds to 1 (100\%), which is exactly what normalizing $P(\Omega)=1$ encodes.
\end{block}
\end{frame}