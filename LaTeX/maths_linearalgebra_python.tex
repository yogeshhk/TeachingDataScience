...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Matrices in NumPy}
\begin{block}{Think About It}
Why does NumPy's built-in matrix multiplication run so much faster than a hand-written Python loop like the ones we built for \lstinline|dot| and \lstinline|vector_sum|?
\end{block}
\pause
\begin{block}{Answer}
NumPy pushes the loop down into pre-compiled C code and uses vectorized CPU operations, instead of looping element-by-element in interpreted Python. Same math, but the "for" loop moves from slow Python bytecode to fast native code.
\end{block}
\end{frame}
