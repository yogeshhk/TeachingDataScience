...
\begin{lstlisting}
# Two-hidden-layer MLP: obs_dim -> 64 -> 64 -> act_dim
# Tanh keeps activations in (-1,1), suitable for continuous action spaces
pi_net = nn.Sequential(
              nn.Linear(obs_dim, 64),
              nn.Tanh(),
              nn.Linear(64, 64),
              nn.Tanh(),
              nn.Linear(64, act_dim)  # outputs raw action logits / values
            )

# Convert numpy observation to float tensor and run one forward pass
