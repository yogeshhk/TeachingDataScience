...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: Regular Expressions}
\begin{block}{Intuition}
A regex is like a find-and-replace search template with wildcards: instead of searching for one
exact string, you're searching for a whole \emph{family} of strings that share a shape (``any
three digits, a dash, then four more digits''), described in a compact mini-language rather than
spelled out one example at a time.
\end{block}
\end{frame}
...
\begin{itemize}
\item Regular expressions are compiled into \lstinline|Pattern| instances,
...
\lstinline{Pattern} instances have several
...
can be found.  
\item If they're successful, a \lstinline|Match| instance is
...
\frametitle{Performing Matches } 
 ...
 Now you can query the \lstinline|Match| for information about the
matching string.  \lstinline|Match| instances also have several
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Regular Expressions}
\begin{block}{Think About It}
Why does \lstinline|re.match(r'dogs', line)| return \texttt{None} on the string
\texttt{"Cats are smarter than dogs"}, while \lstinline|re.search(r'dogs', line)| finds it?
\end{block}
\pause
\begin{block}{Answer}
\lstinline|match()| only tries to match starting at position 0 of the string -- since
\texttt{"Cats..."} doesn't start with \texttt{"dogs"}, it fails immediately.
\lstinline|search()| scans the whole string for the first position where the pattern matches,
so it finds \texttt{"dogs"} wherever it occurs. This is exactly the distinction the ``Matching
versus Searching'' slide demonstrates.
\end{block}
\end{frame}
