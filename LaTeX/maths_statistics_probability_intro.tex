...
These three rules just pin down what "probability" is allowed to mean: never negative, never more than 100\% likely, and something from the sample space always happens. Every other probability rule you'll see can be derived from these three.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Probability Foundations}
\begin{block}{Think About It}
You roll a die. Event $E1$ = "result is even" and $E2$ = "result is at most 3." Are $E1$ and $E2$ disjoint? Does $P(E1 \text{ or } E2) = P(E1) + P(E2)$?
\end{block}
\pause
\begin{block}{Answer}
Not disjoint -- the number 2 satisfies both. So you cannot simply add the probabilities; you must subtract the overlap: $P(E1 \text{ or } E2) = P(E1) + P(E2) - P(E1 \text{ and } E2)$.
\end{block}
\end{frame}
