...
Before starting, verify:
  \begin{itemize}
    ...
    \item A Groq API key: \href{https://console.groq.com}{\texttt{console.groq.com}} (free tier available)
...
export ANTHROPIC_API_KEY="sk-ant-..."  # or your preferred LLM
\end{lstlisting}

\textbf{Install Python deps we'll use:}
\begin{lstlisting}
pip install langchain langchain-groq streamlit rich pytest
...
mkdir mychatbot && cd mychatbot && git init
...
\textbf{Pro Tip:} You can also use \texttt{anthropic/claude-sonnet-4-5} for planning-heavy tasks and switch to a Groq model for speed. OpenCode lets you change models mid-session with \texttt{/models}.
...
OpenCode scans the directory. Since it's empty, it will ask you to describe the project. Tell it:
\begin{lstlisting}
...
OpenCode writes \texttt{AGENTS.md} — commit it to Git. Every future session reads it automatically.
\begin{lstlisting}
