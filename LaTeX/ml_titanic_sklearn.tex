...
Clean the test data the same way as the training data. \texttt{clean\_data()} is just a
function that bundles up every step we did by hand above (map Sex/Embarked to numbers,
fill missing Age, add FamilySize, drop unused columns):
\begin{lstlisting}
...
83\% on the held-out 20\% split. Why the big gap, and which number should you actually
trust to predict how the model will do on Kaggle's real test set?
\begin{enumerate}
  \item[A.] The 83\% number is wrong; the model is really 98\% accurate, the split just got
  unlucky.
  \item[B.] The 98\% reflects memorization of data the model already saw during training; the
  83\% reflects performance on unseen data, which is what actually matters.
...
memorization, not learning. The 83\% figure, measured on the 179 rows held out of training,
...
\begin{block}{Think About It}
Recall is 0.89 for ``Not Survived'' but only 0.72 for ``Survived'', even though both classes
