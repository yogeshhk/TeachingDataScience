...
\end{center}
$V$ contains Singular vectors and $\sum$ contains singular values.
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: SVD}
\begin{block}{Think About It}
$U$ links users to hidden ``concepts'' (like Sci-fi vs. Romance) and $V$ links those same
concepts to movies, with $\Sigma$ giving each concept's strength. If you kept only the single
strongest concept and dropped the rest, what would that do to your ability to predict a
user's rating for an unseen movie, and why might that still be useful despite the lost
information?
\end{block}
\pause
\begin{block}{Answer}
Keeping only the dominant concept collapses every user and movie onto a single axis, so
predictions become coarser -- you'd lose distinctions the weaker concepts captured (e.g. a
secondary comedy/drama axis). But because the strongest concept explains the most variance
in the ratings, this drastically shrinks and de-noises the data while still capturing the
dominant pattern -- the same rank-reduction idea behind collaborative-filtering recommenders.
\end{block}
\end{frame}
