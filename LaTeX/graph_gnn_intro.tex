...
\begin{columns}
    ...
    \begin{itemize}
		...
		\item Graph structure learning: Learn or infer the graph topology from data when the graph is not explicitly observed
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Message Passing: The Core GNN Idea}
...
\item \textbf{Aggregate} — collect representations from neighbours:
	...
	\emph{(Think: each node ''listens to'' its neighbours.)}

\item \textbf{Update} — combine the aggregated signal with the node's own state:
	$$h_v^{(k)} = \mathrm{UPDATE}^{(k)}\!\bigl(h_v^{(k-1)},\; a_v^{(k)}\bigr)$$
	\emph{(Think: each node ''updates its opinion'' based on what it heard.)}
\end{itemize}

After $K$ layers, $h_v^{(K)}$ encodes information from the $K$-hop neighbourhood of $v$. GCN, GAT, GraphSAGE, and MPNN all instantiate \textsc{Aggregate} and \textsc{Update} differently.
