...
\item Lines starting with \texttt{>>>} are typed; the line below is what Python prints back
\end{itemize}
\begin{lstlisting}
...
\begin{itemize}
\item The service is the running program; its API is the way other programs call it, over HTTP
...
\begin{itemize}
\item Send the features as JSON (a plain-text format for data); the service answers with JSON
\item Any language or tool can be the client: a web page, a PLC gateway, a spreadsheet macro
\item Test the bad-input case too: a text value for \texttt{engine\_size} returns HTTP 422 (the request is understood, but its data is invalid)
\item In production, protect the API: require a key or a login, and limit the number of requests
\end{itemize}
\begin{lstlisting}
