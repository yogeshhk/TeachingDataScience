...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: The Parsed HTML Tree}
\begin{block}{Intuition}
Think of parsed HTML as a set of nested boxes: \texttt{<html>} is the outermost box, containing
\texttt{<head>} and \texttt{<body>} boxes, which contain \texttt{<p>} boxes, and so on.
\texttt{.children}, \texttt{.parent}, and calls like \texttt{.find('p')} are just ways of asking
``what's inside this box'' or ``which box contains this one'' -- no different from browsing
folders and subfolders on a hard drive.
\end{block}
\end{frame}
...
\begin{frame}[fragile]\frametitle{HTML}
    ...
    \item Classes and ids are optional, and not all elements will have them. As you can see below, adding classes and ids doesn't change how the tags are rendered at all:
    \end{itemize}
    \begin{lstlisting}
...
\begin{frame}[fragile]\frametitle{Parsing a page with BeautifulSoup}
    ...
    \item Note that children returns a list generator, so we need to call the list function on it. There are two tags at the top level of the page -- the initial \lstinline|<!DOCTYPE html>| tag, newline and the \lstinline|<html>| tag:
    \end{itemize}
        \begin{lstlisting}
list(soup.children)
['html', '\n', <html>
...
\begin{frame}[fragile]\frametitle{Finding all instances of a tag at once}
 If we want to extract a single tag, we can instead use the find\_all method, which will find all
 the instances of a tag on a page. (If you only want the first instance of a tag, use the find
 method instead, which returns a single BeautifulSoup object.)
        \begin{lstlisting}
soup = BeautifulSoup(page.content, 'html.parser')
soup.find_all('p')
[<p>Here is some simple content for this page.</p>]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Using CSS Selectors}
The \lstinline|.select()| method takes an actual CSS selector string, rather than separate tag
and class arguments -- here, \lstinline|p.outer-text| finds every \texttt{p} tag with class
\texttt{outer-text}, the same result as the \lstinline|find_all| call from the previous slide:
            \begin{lstlisting}
...
Inside the forecast item tonight is all the information we want. There are 4 pieces of information we can extract:
    ...
    \item        Use a list comprehension to call the get\_text method on each BeautifulSoup object -- this gets us each of the period names, in order:
	    \end{itemize}
	                \begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Web Scraping}
\begin{block}{Think About It}
This deck fetches a weather page and calls \lstinline|.find(class_='tombstone-container')| for
a single item, then later \lstinline|.select('.tombstone-container .period-name')| for
\emph{all} items. Why the two different methods?
\end{block}
\pause
\begin{block}{Answer}
\lstinline|.find()| stops at the first matching tag -- perfect for grabbing one specific known
element (``tonight's forecast''). \lstinline|.select()| uses CSS-selector syntax and returns
\emph{every} match as a list, which is what you need once you're pulling the same field (period
name, temperature, ...) out of every one of the 9 forecast items via a list comprehension.
\end{block}
\end{frame}
