...
- Validation rules: not-watertight, non-manifold edges,
  degenerate faces
- cli.py: inspect <file> [--report out.json]
...
- src/stlinspector/{core,cli,app,config}.py
- Data flow: file -> core.inspect() -> InspectionReport
- What core.py must NOT depend on (no Streamlit imports)
Show the plan before creating any files.
--
## Approve ("Yes, clear context and auto-accept edits")

\lstinline|Shift+Tab   # Enter Normal mode|

-- Prompt --
Create the stlinspector scaffold with stub methods:
- src/stlinspector/__init__.py
- src/stlinspector/core.py    (stub: inspect(path))
- src/stlinspector/cli.py     (stub: main())
- src/stlinspector/app.py     (stub: Streamlit skeleton)
- src/stlinspector/config.py  (stub: Config dataclass)
...
\begin{lstlisting}
@file:src/stlinspector/core.py
@file:docs/specs/stlinspector-spec.md
Implement core.py:
- load_mesh(path: str) -> trimesh.Trimesh
- inspect(path: str) -> InspectionReport, where InspectionReport is a dataclass with: bounding_box, volume, surface_area, triangle_count, issues: list[str]
- Issue checks:
...
---
## Ask Claude to explain a choice:
Why check edge-sharing counts instead of relying only on
mesh.is_watertight for non-manifold detection?
---
git add src/stlinspector/core.py
...
\begin{lstlisting}
## CLI:
@file:src/stlinspector/cli.py
Implement the `inspect` CLI entry point:
- inspect(path) [--report out.json] via argparse
...
- Also write a human-readable Markdown report to
  reports/<stem>.md (same fields + issues list) -- this
  feeds the inspection-report-summary skill later

---
## Streamlit:
@file:src/stlinspector/app.py
Implement the Streamlit app:
- st.file_uploader restricted to .stl
- Save upload to a temp file, call core.inspect()
...
python src/stlinspector/cli.py tests/fixtures/box.stl --report out.json
...
description: >
  Summarizes an stlinspector InspectionReport (Markdown)
  for a non-technical stakeholder. Use when asked to
  "summarize", "explain", or "triage" an inspection
  report.

## Input
Read the most recent file under reports/*.md (or the
path given): bbox, volume, surface_area, triangle_count,
...
## Usage:
Summarize reports/box.md for a non-technical reviewer
\end{lstlisting}
\end{frame}
...
pytest tests/ --cov=src/stlinspector --cov-report=term-missing
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% DevOps Subagent -- not relevant for this session, disabled for now
\iffalse
\begin{frame}[fragile]\frametitle{DevOps Subagent}

  \begin{itemize}
	\item CI Pipeline
	\item Write \lstinline|.claude/agents/devops.md | as below.
	\item Use the devops subagent to create .github/workflows/ci.yml: on push/PR, set up Python 3.11, pip install -e ".[dev]", run ruff check, run
pytest with coverage, fail the job on any failure.
	\item Also finalise pyproject.toml with an `inspect` console script entry point mapped to stlinspector.cli:main.
  \end{itemize}
...
# Also writes reports/box.md (Markdown report)
...
\begin{frame}[fragile]\frametitle{Context Hygiene}
  ...
  \end{itemize}
\begin{lstlisting}[language=bash]
@file:src/stlinspector/core.py   # Not the whole src/ tree
/compact
\end{lstlisting}
\end{frame}
