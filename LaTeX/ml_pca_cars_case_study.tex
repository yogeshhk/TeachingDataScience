...
\begin{itemize}
\item 8 numeric specs for 426 cars: engine size, cylinders, horsepower, city and highway MPG, weight, wheelbase, length
\item Many features move together: Weight correlates 0.81 with engine size and $-0.79$ with highway MPG
\item Clean first: 2 rotary-engine cars have a dot (\texttt{.}) for cylinders and are dropped
\end{itemize}
\begin{lstlisting}
import pandas as pd
