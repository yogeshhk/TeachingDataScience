...
Pros
\begin{itemize}
\item The most attractive features of the K-nearest neighbor algorithm is that it is simple to understand and easy to implement.
...
\begin{block}{Think About It}
Suppose you train a KNN model to predict house prices using square footage (values in the
...
\begin{frame}[fragile]\frametitle{Quick Check: K-Nearest Neighbor}
\begin{block}{Answer}
Square footage will dominate, since distances are computed on raw magnitudes and a
difference of a few hundred square feet dwarfs a difference of one bedroom. KNN is sensitive
to this because every feature contributes directly to a single geometric distance number;
a Decision Tree instead considers one feature at a time when choosing splits, so an
unscaled feature doesn't drown out the others the way it does for a distance metric.
\end{block}
\end{frame}
