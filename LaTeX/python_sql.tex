...
Let's say we have a CSV file that holds students and their grades in certain courses. Both
students took the same courses; student 1 did pretty well, student 2 not as well. Grades are
assigned a numerical value with A being a 4, B being a 3, etc.:
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile] \frametitle{Intuition: GROUP BY}
\begin{block}{Intuition}
Think of GROUP BY as sorting rows into labeled buckets -- one bucket per distinct value (or
combination of values). An aggregate function like SUM or AVG then collapses each bucket down
to a single number, so the query returns one row per bucket, not one row per original row.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile] \frametitle{Grouping and Aggregate Queries}
This takes the total gradepoints of each group and divides them by the total credits in that
group. Since the grouping is by studentid, each student is a group, and each group contains the
rows for that student, so any aggregate functions operate on the rows for each student
independently:
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile] \frametitle{Joins}
We can filter this out by selecting only where the studentid column of the first table is equal to the studentid column of the second table. This is called a join condition and will get us the demographics and grades of the respective students. This performs a regular join, also known as an inner join or cross join -- only rows that match the join condition are returned:
\begin{lstlisting}
...
Joins work with groups just as you'd expect. All joins happen before grouping, so you can get a
student's demographic information along with their GPA in one query (if you're wondering why we
couldn't just put the demographics data into the grades table, there are a couple of reasons --
covered next):
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile] \frametitle{Quick Check: SQL Joins}
\begin{block}{Think About It}
Why does a bare \lstinline|JOIN| (no \lstinline|ON|/\lstinline|WHERE| condition) between the
10-row grades table and the 3-row demographics table return 30 rows, while adding
\lstinline|ON g.studentid=d.studentid| cuts that down dramatically?
\end{block}
\pause
\begin{block}{Answer}
A bare \lstinline|JOIN| performs a full cartesian product -- every row of one table paired with
every row of the other, regardless of whether they're related, giving $10 \times 3 = 30$ rows.
Adding a join condition filters that cartesian product down to only the pairs where the
specified columns actually match, which is what makes the join meaningful: each grade row
matched to the \emph{one} demographics row for that same student, not all three.
\end{block}
\end{frame}
