...
The t-score answers: "is the gap between the groups big compared to the natural noise inside each group?" A big gap between two groups means little if each group is itself wildly scattered (high within-group variation swamps the signal) -- t-score captures exactly this signal-to-noise idea.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: t-Tests}
\begin{block}{Think About It}
You want to compare exam scores of the same 30 students before and after a new teaching method. Should you use a paired t-test or an unpaired (independent samples) t-test?
\end{block}
\pause
\begin{block}{Answer}
Paired t-test -- the same subjects are measured twice (before/after), so the two sets of scores are linked rather than independent. Pairing lets the test focus on each student's own change, which removes person-to-person variability from the noise.
\end{block}
\end{frame}
