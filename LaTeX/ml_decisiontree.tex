...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Game: the Rules}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Game: Eliminating Subgroups}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Game: It's a Tree}
Its a tree.
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Game: What is Success?}
Whats the success?
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Approach: First Split, on JEE Score}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Approach: First Split, Illustrated}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Approach: Second Split, on 12th Percentage}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Approach: Second Split, Illustrated}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Approach: Splitting the Less-Likely Group}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Approach: the Completed Tree}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Questions: Does Split Order Matter?}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Questions: How Many Attributes to Use?}
\begin{itemize}
	\item What if we used more criteria: essay scores, extra-curriculars, sports awards?
	\item How many attributes should we use to create splits?
	\item Which attributes are the most significant?
	\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Decision Trees: What They Are}
\begin{itemize}
	\item  Decision Trees (DTs) are a non-linear method for supervised learning, used for classification and regression.
	\item Goal: build a model that predicts a target variable.
	\item It learns simple decision rules from the data features.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Decision Trees: As If-Then-Else Rules}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Decision Trees: a Simple, Widely-Used Technique}
...
\begin{tikzpicture}[scale=0.62, every node/.style={draw,rounded corners,align=center,font=\tiny}]
\node[fill=violet!20] (root) at (0,3) {Root};
\node[fill=violet!10] (int1) at (-1.3,1.5) {Internal};
\node[fill=violet!10] (int2) at (1.3,1.5) {Internal};
\node[fill=gray!20] (leaf1) at (-2,0) {Leaf};
\node[fill=gray!20] (leaf2) at (-0.6,0) {Leaf};
\node[fill=gray!20] (leaf3) at (0.6,0) {Leaf};
\node[fill=gray!20] (leaf4) at (2,0) {Leaf};
\draw[->] (root) -- (int1);
\draw[->] (root) -- (int2);
\draw[->] (int1) -- (leaf1);
\draw[->] (int1) -- (leaf2);
...
\end{center}
\tiny{Root $\to$ Internal (questions) $\to$ Leaf (class label)}
\end{minipage}%
}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Decision Trees: Rise of Ensembles}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{John Play Tennis: the Dataset}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{John Play Tennis: Splitting Steps}
Steps:
	\begin{itemize}
...
Let's start with the first column, ``Outlook''.
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{John Play Tennis: Split on Outlook}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{John Play Tennis: Outlook-Overcast Turns Pure}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{John Play Tennis: Outlook-Sunny Turns Pure}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{John Play Tennis: Outlook-Rain Turns Pure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{John Play Tennis: Reading the Final Tree}
...
\begin{frame}[fragile]\frametitle{Which attribute to split on}
	...
	\item Are we more certain about Yes/No after the split?
		...
		\end{itemize}
	\item Must be symmetric: 4 yes / 0 no is just as pure as 0 yes / 4 no.
	\end{itemize}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy: Intuition}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy: Shannon's Formula}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy: Origin in Thermodynamics}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy: Binary Outcome Formula}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy: Applying it to Our Example}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy: Computing $S = [9+,5-]$}
Recall our Play-Tennis dataset: 9 Yes, 5 No, so $S = [9+,5-]$.
\begin{center}
\footnotesize
$\displaystyle Entropy(S) = -\left(P_\oplus \log_2 P_\oplus + P_\ominus \log_2 P_\ominus\right)$
\end{center}
Where $P_\oplus$ is the portion of positive examples and $P_\ominus$ is the portion of negative examples in $S$.
\begin{center}
\footnotesize
$\displaystyle Entropy([9+,5-]) = -\left(\frac{9}{14}\log_2\frac{9}{14} + \frac{5}{14}\log_2\frac{5}{14}\right) = 0.940$
\end{center}
Which concludes, the dataset is 94\% impure or 94\% non-homogeneous.
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Information Gain: Splitting on Wind}
$S = [9+,5-]$, \quad $S_{weak} = [6+,2-]$, \quad $S_{strong} = [3+,3-]$, \quad $Entropy(S) = 0.940$
\begin{center}
\footnotesize
...
& - \left(\frac{8}{14}Entropy(S_{weak}) + \frac{6}{14}Entropy(S_{strong})\right)
\end{aligned}$
\end{center}
\begin{center}
\footnotesize
$\displaystyle Entropy(S_{weak}) = -\left(\frac{6}{8}\log_2\frac{6}{8} + \frac{2}{8}\log_2\frac{2}{8}\right) = 0.811$
\end{center}
\begin{center}
\footnotesize
$\displaystyle Entropy(S_{strong}) = -\left(\frac{3}{6}\log_2\frac{3}{6} + \frac{3}{6}\log_2\frac{3}{6}\right) = 1.00$
\end{center}
Put the values of $Entropy(S_{weak})$ and $Entropy(S_{strong})$ in the Gain equation:
\begin{center}
\footnotesize
$\displaystyle \begin{aligned}
Gain(S,Wind) = {}& 0.940 - \left(\frac{8}{14}(0.811) + \frac{6}{14}(1.00)\right) \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Information Gain: Splitting on Outlook}
$Values(Outlook) = \{Sunny, Overcast, Rain\}$, \quad $S = [9+,5-]$
\begin{center}
\footnotesize
$\displaystyle S_{sunny} = [2+,3-], \quad S_{overcast} = [4+,0-], \quad S_{rain} = [3+,2-]$
\end{center}
\begin{center}
\footnotesize
$\displaystyle \begin{aligned}
G(S,Outlook) = {}& Entropy(S) \\
& - \frac{5}{14}Entropy(S_{sunny}) \\
& - \frac{4}{14}Entropy(S_{overcast}) \\
& - \frac{5}{14}Entropy(S_{rain})
\end{aligned}$
\end{center}
\begin{center}
\footnotesize
$\displaystyle Entropy(S_{sunny}) = -\left(\frac{2}{5}\log_2\frac{2}{5} + \frac{3}{5}\log_2\frac{3}{5}\right) = 0.971$
\end{center}
\begin{center}
\footnotesize
$\displaystyle Entropy(S_{overcast}) = -\left(\frac{4}{4}\log_2\frac{4}{4} + \frac{0}{4}\log_2\frac{0}{4}\right) = 0$
\end{center}
\begin{center}
\footnotesize
$\displaystyle Entropy(S_{rain}) = -\left(\frac{3}{5}\log_2\frac{3}{5} + \frac{2}{5}\log_2\frac{2}{5}\right) = 0.971$
\end{center}
Put the values of $Entropy(S_{sunny})$, $Entropy(S_{overcast})$, $Entropy(S_{rain})$ in the equation above.
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy Example: Stars vs Diamonds Setup}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy Example: Splitting by Color}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy Example: Full-Group Entropy}
...
\footnotesize
$\displaystyle \begin{aligned}
E(\vec{d}) = {}& -\sum_i p_i \log_2(p_i) = -\left(p_1\log_2(p_1) + p_2\log_2(p_2)\right) \\
= {}& -\left(\frac{24}{49}\log_2\left(\frac{24}{49}\right) + \frac{25}{49}\log_2\left(\frac{25}{49}\right)\right) \cong 0.9997
\end{aligned}$
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy Example: Orange and Blue Group Entropy}
...
= {}& -\left(\frac{4}{25}\log_2\left(\frac{4}{25}\right) + \frac{21}{25}\log_2\left(\frac{21}{25}\right)\right) \cong 0.6343
\end{aligned}$
\end{center}
Blue Group:
\begin{center}
...
= {}& -\left(\frac{21}{24}\log_2\left(\frac{21}{24}\right) + \frac{3}{24}\log_2\left(\frac{3}{24}\right)\right) \cong 0.5436
\end{aligned}$
\end{center}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Entropy Example: Combined Entropy and Information Gain}
\begin{itemize}
...
\footnotesize
$\displaystyle \begin{aligned}
E(\vec{d},\vec{a}) = {}& \frac{25}{49}E(orange) + \frac{24}{49}E(blue) \\
...
\begin{center}
\footnotesize
$\displaystyle I(\vec{d},\vec{a}) = E(\vec{d}) - E(\vec{d},\vec{a}) = 0.9997 - 0.5899 = 0.4097$
\end{center}
Substantial gain.
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Gini Index: Example Setup}
\begin{center}
...
\item   $Gini < 4 = 0.4081$
\item $Gini > 4 = 0$
\item Gini Gain = 0.4687 - 10/16 (0.4081) - 6/16 (0)
...
Both formulas answer the same question: ``how mixed is this node?'', just with different math. Entropy's $\log_2$ term is expensive to compute at every candidate split of every node, while Gini's plain $p^2$ is just a multiplication, and much cheaper. That is why CART (and scikit-learn's default) uses Gini even though the two rarely disagree on which split is best.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Compared to Linear Models: Linear Boundary}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Compared to Linear Models: Nonlinear Boundary}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Decision Trees}
...
\begin{frame}[fragile]\frametitle{Quick Check: Decision Trees}
\begin{block}{Answer}
It helps as long as the trees' \emph{errors} are not all the same. A single fully-grown tree overfits to the specific noise/quirks of its training sample, and different random samples (or subsets of features) produce trees that overfit to \emph{different} noise. Real signal tends to agree across trees; noise-driven mistakes tend to cancel out when averaged, because they point in different, uncorrelated directions. This is exactly why Random Forest deliberately randomizes both the training rows (bagging) and the candidate features per split: decorrelating the trees' errors is the whole point, not an accident.
\end{block}
\end{frame}
