...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: Why Object-Oriented Matplotlib?}
\begin{block}{Intuition}
Plain \lstinline|plt.plot()| calls are like talking to ``whichever plot is currently active'' --
convenient for one simple plot, but ambiguous once there's more than one. The object-oriented
style (\texttt{fig}, \texttt{ax}) addresses a specific plot by name, the same way you'd address a
specific person in a room instead of just shouting -- essential once a figure has multiple
subplots, as the next slide shows.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Plotting}
\begin{block}{Think About It}
Why does \lstinline|plt.plot(x, y1); plt.plot(x, y2); plt.show()| put both curves on ONE set of
axes, while the \lstinline|fig, (ax1, ax2) = plt.subplots(...)| version puts them on two separate
axes?
\end{block}
\pause
\begin{block}{Answer}
\lstinline|plt.plot()| is a state-machine call that implicitly draws on ``whichever axes is
currently active'' -- by default there's only one, so both calls land on it. The object-oriented
style makes the axes explicit (\texttt{ax1}, \texttt{ax2}), so each \lstinline|.plot()| call is
unambiguous about which subplot it targets.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Histogram Example}
