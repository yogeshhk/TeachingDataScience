...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: Lambda}
\begin{block}{Intuition}
A lambda is like a sticky note version of a function: quick to jot down and use once, right
where you need it -- but not meant to be filed away with a proper name and referenced later the
way a \texttt{def} function (a formal document) would be.
\end{block}
\end{frame}
...
\begin{frame}[fragile]\frametitle{Lambda Usage}
	...
	\item map expects a function object and any number of iterables like list, dictionary, etc.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Lambda Example}
Iterating over a dictionary using map and lambda. Each dict of dict\_a will be passed as
parameter to the lambda function, and the result of the lambda expression for each dict will be
given as output (wrapped in \lstinline|list()| since \lstinline|map()| is lazy):
	    \begin{lstlisting}
...
We can pass multiple sequences to the map functions as shown below. Each ith element of list\_a
and list\_b will be passed as argument to the lambda function:
	    \begin{lstlisting}
list_a = [1, 2, 3]
list_b = [10, 20, 30]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Lambda \& map/filter}
\begin{block}{Think About It}
Both \lstinline|map(f, seq)| and \lstinline|[f(x) for x in seq]| (a list comprehension) apply
\texttt{f} to every element. In Python 3, when might you actually prefer \lstinline|map()|
despite list comprehensions generally being considered more Pythonic?
\end{block}
\pause
\begin{block}{Answer}
\lstinline|map()| is lazy, like a generator -- it computes nothing until you iterate it, so
it's the right choice when the sequence is huge or you only need to consume part of it (e.g.
combined with \lstinline|itertools.islice|). A list comprehension eagerly builds the whole list
in memory immediately. If you do need the full list right away, though, most style guides still
prefer the comprehension for readability.
\end{block}
\end{frame}
