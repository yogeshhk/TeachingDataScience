...
Prompts (LLMs) & a prompt change is a release & prompt registry \\
\hline
\end{tabular}
\end{center}
...
Model & prediction drift, accuracy when labels arrive, per-group quality & MAE on last month \\
System & latency, throughput, errors, cost & p95 response time \\
Business & the KPI the model exists to improve & fewer unplanned stops \\
\hline
\end{tabular}
\end{center}
...
1. A drift alert fires on the input data of a deployed model, but labels for recent data have not arrived. What is the best first step?
\begin{enumerate}
...
\end{enumerate}
2. A team edits a prompt in production and answer quality silently drops. Which practice would most likely have caught it before release?
\begin{enumerate}
  \item[A.] Editing prompts directly in the running application for speed
  \item[B.] Versioning prompts and running them against a golden set of test cases before release
  \item[C.] Buying a bigger GPU
  \item[D.] Retraining the base model every week
\end{enumerate}
\end{block}
\end{frame}
...
\begin{frame}[fragile]\frametitle{Quick Check: Production Practice}
\begin{block}{Answer}
\textbf{1. Correct: B.} A drift alert says the inputs changed, not that the model is wrong: it may be a broken sensor or pipeline. Investigate first, and if the change is real, retrain and release gradually (A skips the checks, C waits for damage, D removes the fallback).

\textbf{2. Correct: B.} A prompt is part of the release. Versioning it and testing it on a golden set catches regressions before users do. Hardware (C) and base-model retraining (D) do not address a prompt edit.
\end{block}
\end{frame}
...
\item \href{https://mlflow.org/docs/latest/genai/prompt-registry/}{MLflow: Prompt Registry}
\item \href{https://lakefs.io/mlops/mlops-tools/}{lakeFS: 26 MLOps Tools for 2026}
\item \href{https://lakefs.io/blog/llm-observability-tools/}{lakeFS: LLM Observability Tools, 2026 Comparison}
