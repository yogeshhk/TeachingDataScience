...
Standard deviation is the "typical distance" a value sits from the mean. Squaring the deviations (instead of just averaging them directly) stops positive and negative differences from cancelling out, and taking the square root at the end brings the units back to match the original data.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Measures of Spread}
\begin{block}{Think About It}
Two datasets have the same mean and range, but Dataset A has a much smaller standard deviation than Dataset B. What does that tell you about how the values are distributed within each?
\end{block}
\pause
\begin{block}{Answer}
Dataset A's values are clustered tightly around the mean, while Dataset B's values are more spread out -- even though both share the same overall range and center, standard deviation reveals how "typical" values are for the whole dataset, which the range alone (only min/max) cannot show.
\end{block}
\end{frame}
