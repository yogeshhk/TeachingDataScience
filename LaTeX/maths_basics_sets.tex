...
Just adding $|A|+|B|+|C|$ double-counts anything shared between two sets, so we subtract the pairwise overlaps. But that subtracts the triple-overlap region three times over, so we add it back once. This "add, subtract, add back" pattern is called inclusion-exclusion.
\end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Sets}
\begin{block}{Think About It}
If $A$ and $B$ are disjoint, what does the formula $|A\cup B|=|A|+|B|-|A\cap B|$ simplify to, and why does that make intuitive sense?
\end{block}
\pause
\begin{block}{Answer}
It simplifies to $|A\cup B|=|A|+|B|$, since disjoint sets have $|A\cap B|=0$ -- nothing is shared, so nothing needs to be subtracted for double-counting.
\end{block}
\end{frame}
...
\begin{block}{Intuition}
Don't memorize Group/Ring/Field as separate facts -- see them as a ladder of "how much structure does this set have?" A Group can do one operation cleanly (like addition). A Ring adds a second operation (multiplication) that plays nicely with the first. A Field is a Ring where you can also divide (except by zero). Real numbers are a Field; integers are only a Ring (no division that stays an integer).
\end{block}
...
The ordered field ${\mathbb Q}$ of rational numbers, on the other hand,
is \textbf{not} complete.

The complex numbers ${\mathbb C}$ also form a complete field.

\begin{block}{Intuition}
Completeness means "no gaps": if you approach a boundary from below and never overshoot, you actually reach a real number at that boundary. Rationals have gaps -- you can get arbitrarily close to $\sqrt{2}$ using rationals, but $\sqrt{2}$ itself is missing from $\mathbb{Q}$.
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Algebraic Structures}
\begin{block}{Think About It}
The integers $\mathbb{Z}$ form a Ring but not a Field. Which Field property fails, and can you give a concrete example?
\end{block}
\pause
\begin{block}{Answer}
Multiplicative inverses fail to stay in the set: $2 \in \mathbb{Z}$, but its inverse $1/2 \notin \mathbb{Z}$. A Field requires every nonzero element to have an inverse within the same set.
\end{block}
\end{frame}