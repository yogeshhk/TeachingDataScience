...
\begin{block}{Recent Architecture Update}
LangChain's core library has been streamlined to focus on:
\begin{itemize}
\item RAG building blocks: Loaders, Splitters, Embeddings, Retrievers
\item LCEL chains for composable, stateless workflows
\item Simple agent patterns with standard tool use
\end{itemize}
Complex stateful workflows (loops, multi-agent systems, persistence)
are now the domain of \textbf{LangGraph} --- a separate library built
on top of LangChain that provides graph-based state orchestration.
\end{block}  
\end{frame}
...
\begin{frame}[fragile]\frametitle{}
\begin{center}
{\Large Summary: What's New in LangChain Ecosystem}
...
\centering
\begin{table}
\begin{tabular}{| p{1.4cm} | p{2.75cm} | p{2.75cm} | p{2.75cm} |}
\hline
