...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Wrapping Up}
\begin{block}{Think About It}
Which Zen of Python line best explains why this workshop pushed \lstinline|with open(...) as f:|
over manual \lstinline|open()|/\lstinline|close()|, and \lstinline|try|/\lstinline|except| over
letting errors crash silently?
\end{block}
\pause
\begin{block}{Answer}
``Errors should never pass silently, unless explicitly silenced'' and ``Explicit is better than
implicit'' both apply: \texttt{with} makes resource cleanup explicit and guaranteed rather than
implicitly hoped-for, and explicit exception handling means failures are seen and dealt with
instead of silently corrupting data (like an unflushed file write).
\end{block}
\end{frame}
