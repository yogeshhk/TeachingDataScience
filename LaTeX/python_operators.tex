...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile] \frametitle{Operators}
Use assignment `\texttt{=}' statement, with shortcut notations for compound updates:
  \begin{itemize}
...
Tests whether two names refer to the \emph{same object in memory} (identity, not value equality):
\begin{alertblock}{Use \texttt{==} to compare values}
\texttt{is} tests memory identity. The only idiomatic use of \texttt{is} in Python is \lstinline|if x is None:|
\end{alertblock}
\begin{lstlisting}
>>> a = 1
>>> b = 1
>>> a is b
True         # works here: CPython caches small integers (-5 to 256)

>>> a = 1000
>>> b = 1000
>>> a is b
False        # different objects even though values are equal!
\end{lstlisting}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Operators}
\begin{block}{Think About It}
\texttt{10 / 3} and \texttt{10 // 3} both divide 10 by 3, but Python keeps them as two separate operators rather than one ``divide'' operator with a rounding flag. Why?
\end{block}
\pause
\begin{block}{Answer}
Because they promise different things: \texttt{/} (true division) always returns a float -- ``here is the exact quotient'' -- while \texttt{//} (floor division) always returns a rounded-down whole value -- ``here is how many whole times this fits.'' Keeping them as distinct operators means you can tell the type of the result just by reading the code, instead of it depending on a runtime flag or the types of the operands.
\end{block}
\end{frame}
