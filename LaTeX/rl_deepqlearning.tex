...
\begin{lstlisting}
 ...
 with tf.variable_scope('model'):
     ...
     # Sample one action per observation from the categorical distribution
     actions = tf.squeeze(tf.multinomial(logits=logits,num_samples=1), axis=1)

 # make loss (policy gradient surrogate objective)
 adv_ph = tf.placeholder(shape=(None,), dtype=tf.float32)
 act_ph = tf.placeholder(shape=(None,), dtype=tf.int32)
 # One-hot encode chosen actions to select their log-probabilities
 ...
 # REINFORCE loss: maximise sum of log_prob * advantage
...
while True:
    ...
    if done:
        ...
        # Compute discounted reward-to-go for each timestep in the episode
        batch_rtgs += list(discount_cumsum(ep_rews, gamma))
        obs, rew, done, ep_rews = env.reset(), 0, False, []
        if len(batch_obs) > batch_size:
            break
# Normalize advantages to reduce variance and stabilise training
batch_advs = np.array(batch_rtgs)
batch_advs = (batch_advs - np.mean(batch_advs))/(np.std(batch_advs) + 1e-8)
# Single gradient step: feed observations, actions, and normalised advantages
