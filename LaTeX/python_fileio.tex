...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: File Modes}
\begin{block}{Intuition}
Think of a file like a book: \texttt{'r'} opens it to read without disturbing a single page;
\texttt{'w'} opens a brand-new blank book, discarding anything that was written before;
\texttt{'a'} opens it to the last page so you keep writing where you left off. Calling
\texttt{close()} is like closing the book -- it makes sure your writing is actually saved to the
page (flushed to disk) and frees the book up for others to use.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{The \texttt{with} Statement}
Every example so far has called \lstinline|open()| and then manually \lstinline|close()|d the
file -- easy to forget, and skipped entirely if an exception is raised in between. The
\texttt{with} statement fixes both problems, and is the idiomatic way to open files in modern
Python: it closes the file automatically as soon as the block ends, even if an exception occurs
inside it:
\begin{lstlisting}
with open(fname) as fhand:
    ...
    for line in fhand:
        if line.startswith('Subject:'):
            count = count + 1
    print('There were', count, 'subject lines in', fname)
# fhand is guaranteed to be closed here, no explicit close() needed
\end{lstlisting}
\end{frame}
...
Unlike \lstinline|os.mkdir|, \lstinline|os.makedirs| is a no-op if the directory already exists,
and creates all the intermediate-level directories needed to contain the leaf:
  \begin{lstlisting}
  os.makedirs(path)
\end{lstlisting}
%\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: File IO}
\begin{block}{Think About It}
You open a file with \lstinline|f = open('data.txt', 'w')|, write some lines, but your script
crashes before calling \lstinline|f.close()|. What happens to your data, and why?
\end{block}
\pause
\begin{block}{Answer}
Writes are often buffered in memory and only guaranteed to reach disk when the file is closed (or
explicitly \lstinline|flush()|ed). If the script crashes first, some or all of the written data
may be lost. This is exactly why the \lstinline|with open(...) as f:| pattern is preferred -- it
closes the file automatically even when an exception is raised.
\end{block}
\end{frame}