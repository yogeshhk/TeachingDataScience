...
\begin{center}
\includegraphics[width=0.8\linewidth,keepaspectratio]{calc8}
\end{center}
...
\begin{center}
\includegraphics[width=0.8\linewidth,keepaspectratio]{calc9}
\end{center}
...
This is just a precise way of saying "zoom in enough near $x_0$, and the curve looks like a straight line." No matter how strict a tolerance ($\epsilon$) you demand, you can always zoom in close enough (find a $\delta$) to meet it -- that's what makes the slope well-defined at that point.
\end{block}
\end{frame}
...
\item $f'(x) = \lim_{h \to 0} \frac{(x+h)^2 - x^2}{h} = \lim_{h \to 0} (2x + h) = 2x$
...
\begin{center}
\includegraphics[width=0.6\linewidth,keepaspectratio]{calc10}
\end{center}
...
\begin{block}{Intuition}
Think of it as a rate-of-change relay race: if $y$ changes 3x as fast as $u$, and $u$ changes 2x as fast as $x$, then $y$ changes $3 \times 2 = 6$x as fast as $x$. The chain rule just multiplies the rates along the chain.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Derivatives}
\begin{block}{Think About It}
At a function's minimum point, what must the first derivative equal? And how would you use the second derivative to confirm it's a minimum and not a maximum?
\end{block}
\pause
\begin{block}{Answer}
The first derivative must be 0 (the tangent line is flat). If the second derivative there is positive, the slope is turning from negative to positive -- curving upward -- confirming a minimum (a negative second derivative would confirm a maximum instead).
\end{block}
\end{frame}
