...
The gradient points "uphill" -- the direction of steepest increase. So stepping in the \emph{opposite} direction is like walking downhill in fog: you can't see the whole landscape, but taking small steps against the slope you feel under your feet reliably gets you to the valley floor (the minimum).
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Optimization}
\begin{block}{Think About It}
If the learning rate in gradient descent is set far too large, what do you expect to happen to the error as training proceeds -- steady decrease, slow decrease, or wild oscillation/divergence?
\end{block}
\pause
\begin{block}{Answer}
Wild oscillation or divergence. A step size too large overshoots the minimum every time, bouncing back and forth (or further away) instead of settling down -- like trying to walk downhill but taking leaps so big you keep landing on the opposite slope.
\end{block}
\end{frame}
