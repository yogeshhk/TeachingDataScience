...
\note{Euler's rule: an Eulerian path exists only if the graph has exactly zero or two vertices of odd degree. In the Königsberg graph all four land-masses have odd degree, so no such walk exists.}
\end{frame}
...
\begin{itemize}
\item Undirected graph: edges are bidirectional, e.g. Michael is brother of John, necessarily means that John is brother of Michael.
\item Directed graph: edges have one direction, e.g. A likes B does not necessarily mean that B likes A.
\item Weighted graph: edges have weights, e.g. connection from city A to city B via road R1 will have weight, say, 8, due to high traffic, but via road R2, may have weight 2, due to lower traffic.
\end{itemize}
