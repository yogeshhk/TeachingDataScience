...
\begin{frame}[fragile]\frametitle{Sets}
  ...
  \item Unordered. 
  \item Variable length, heterogeneous, arbitrarily nest-able (non-mutable tuples can be added as elements, but not mutable lists or sets)
...
\item Do  *not* use the \lstinline|empty_set = {}|
...
\item And so on \ldots
  ...
  A hash table is like a library that shelves books by a formula computed from the title, rather than alphabetically -- given a title, you jump straight to its shelf instead of scanning every book. That's why checking \texttt{x in my\_set} is fast (roughly constant time) no matter how large the set is, unlike scanning a list from the front.
  \end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Dictionaries \& Sets}
\begin{block}{Think About It}
A \texttt{set} and a \texttt{dict}'s keys are both unordered collections of unique, hashable items -- so what's the essential difference between \texttt{\{1, 2, 3\}} (a set) and \texttt{\{1: None, 2: None, 3: None\}} (a dict)?
\end{block}
\pause
\begin{block}{Answer}
Functionally almost nothing when you only care about membership -- a set is really just a dict that only stores keys and throws away the values. The real difference is intent: use a set when you only care ``is this value present?'', and a dict when you need to associate each key with additional information.
\end{block}
\end{frame}