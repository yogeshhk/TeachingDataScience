...
All that algebra boils down to one idea: instead of solving directly for the widest gap, we
rephrase it as ``minimize $||w||$'' -- a smooth, convex problem that optimization software
already knows how to solve reliably. The reward for the detour is that the solution turns
out to depend on only a handful of training points (the support vectors); every other point,
however far from the boundary, could be deleted without changing the answer at all.
\end{block}
\end{frame}
...
\item Solving these two equations simultaneously. From the positive-point equation, $w_0 = 1 - 8a$. Putting it in the negative-point equation: $3a + 1 - 8a = -1$, thus $5a = 2$ so $a = \frac{2}{5}$
\item Putting $a$ back, $ w_0 = 1 - 8\frac{2}{5} = -\frac{11}{5}$
\item Thus , $ \bar{w} = (\frac{2}{5},\frac{4}{5})$
