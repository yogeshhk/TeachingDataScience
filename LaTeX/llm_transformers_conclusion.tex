...
\begin{itemize}
\item \textbf{Positional Encoding:} instead of architecture relying on sequence order, like RNNs, the Transformer stores the position as part of the data itself (sinusoidal vectors), making the architecture order-independent and fully parallelisable.
\item \textbf{Self-Attention (Q, K, V):} Each token broadcasts a Key (``what I offer''), issues a Query (``what I seek''), and delivers a Value (``what I provide''). The three-matrix decomposition allows the model to decouple search relevance from information content, enabling rich contextual representations that RNNs cannot form in parallel.
\item \textbf{Multi-Head Attention:} Running multiple attention heads simultaneously allows the model to capture diverse linguistic relationships (syntax, coreference, proximity) in a single layer.
\item \textbf{Feed Forward Network:} Adds non-linearity per token after attention, giving the model capacity to compute complex transformations beyond linear weighted sums.
\item \textbf{Cross-Attention in Decoder:} Connects the decoder's current state to the encoder's full source representation, enabling conditioned generation for translation and summarisation.
\item \textbf{Causal Masking:} Prevents the decoder from seeing future tokens during training, ensuring training and inference behaviour are aligned.
\item \textbf{Self-Supervised Pre-training:} MLM, CLM, and Seq2Seq denoising allow learning from raw text at scale without human labels.
\end{itemize}
