...
%
%  Tools : LangChain >= 0.2 (community API), langchain-text-splitters,
%           HuggingFace Embeddings, ChromaDB >= 0.5, HuggingFaceHub LLM, RAGAS
%  Data  : local faq.txt
...
\begin{columns}
  \begin{column}[T]{0.6\linewidth}
    \begin{itemize}
      ...
      \item \texttt{TextLoader} will read the entire file into a single LangChain \texttt{Document} object; the splitter then breaks it into per-entry chunks.
    \end{itemize}
  \end{column}
...
\begin{columns}
  \begin{column}[T]{0.6\linewidth}
    \begin{itemize}
      % G4: Updated package list — added langchain-text-splitters (split from
      %     langchain core in 0.2+); updated version numbers to 2025 stable releases.
      \item \textbf{langchain} $\geq$0.2: the community-stable API used throughout.
      \item \textbf{langchain-community}: integrations (loaders, LLMs, vector stores).
      \item \textbf{langchain-text-splitters}: text splitting utilities (new package in 0.2+).
      \item \textbf{sentence-transformers}: local embedding model; no API key needed.
      \item \textbf{chromadb} $\geq$0.5: lightweight local vector store; auto-persists to disk.
...
conda create -n rag-demo python=3.11
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Step 1: Load and Parse with TextLoader}

\begin{columns}
  \begin{column}[T]{0.6\linewidth}
    \begin{itemize}
      \item \textbf{TextLoader} reads a plain-text file and returns a list of LangChain \texttt{Document} objects, each carrying \texttt{page\_content} and \texttt{metadata}.
      \item The file is loaded as a \textbf{single Document}; the text splitter in Step 2 divides it into individual Q\&A chunks.
      \item Each \texttt{Document} automatically receives \texttt{source} metadata (the file path), useful for citations in the final answer.
      \item Printing the first document confirms the content was loaded correctly before proceeding.
    \end{itemize}
  {\tiny (Ref: https://python.langchain.com/docs/integrations/document\_loaders/)}
  \end{column}
  \begin{column}[T]{0.4\linewidth}
\begin{lstlisting}[language=python, basicstyle=\tiny]
# T1d: Updated import namespace - TextLoader
# moved to langchain_community in LangChain 0.2+
...
\begin{columns}
  \begin{column}[T]{0.6\linewidth}
    ...
    \end{itemize}
  {\tiny (Ref: https://python.langchain.com/docs/how\_to/recursive\_text\_splitter/)}
  \end{column}
  \begin{column}[T]{0.4\linewidth}
\begin{lstlisting}[language=python, basicstyle=\tiny]
# T1e: Updated import - RecursiveCharacterTextSplitter
# moved to langchain-text-splitters package in 0.2+
...
\begin{columns}
  \begin{column}[T]{0.6\linewidth}
    ...
    \end{itemize}
  {\tiny (Ref: https://python.langchain.com/docs/integrations/vectorstores/chroma/)}
  \end{column}
  \begin{column}[T]{0.4\linewidth}
\begin{lstlisting}[language=python, basicstyle=\tiny]
...
# T1f: vectorstore.persist() is DEPRECATED in
# ChromaDB >= 0.4.0. Persistence is now automatic
# when persist_directory is set. Remove this call
...
\begin{columns}
  \begin{column}[T]{0.6\linewidth}
    ...
    \end{itemize}
  {\tiny (Ref: https://python.langchain.com/docs/integrations/llms/)}
  \end{column}
  \begin{column}[T]{0.4\linewidth}
\begin{lstlisting}[language=python, basicstyle=\tiny]
# T1g: Updated import namespace - HuggingFaceHub
# moved to langchain_community.llms in LangChain 0.2+
...
\begin{itemize}
  ...
  % G3: Note about modern LangChain API.
  \item \textbf{Modern alternative (LCEL):} LangChain 0.2+ recommends the LangChain Expression Language (\texttt{|} operator) over \texttt{RetrievalQA} for composable, type-safe chains: \texttt{chain = prompt | llm | StrOutputParser()}.
\end{itemize}
