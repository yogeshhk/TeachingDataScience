...
$a(x)$ is just an average (or vote) over $N$ trees -- averaging cancels out each tree's
...
\end{itemize}
\pause
Answer: 96.66\%. With 7 jurors at $p=0.8$ each, the jury is correct whenever 4 or more
agree: $\sum_{k=4}^{7}{7 \choose k}(0.8)^k(0.2)^{7-k} \approx 0.9666$ -- higher than any
single juror's 80\%, illustrating the same jury-theorem effect as the ensemble formula above.
\end{frame}
