...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forests: Origins}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forests: How They Differ from Bagging}
\begin{itemize}
...
\item Due to the random feature selection, the trees are more independent of each other than in regular bagging.
\begin{itemize}
\item Better variance-bias trade-off, so often better predictive performance.
\item Faster than bagging, since each tree learns from only a subset of features.
\end{itemize}
\end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest: Illustrated}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile] \frametitle{Random Forests: Basic Steps}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest Algorithm: Setup}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest Algorithm: Construction}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest Algorithm: Construction, Illustrated}
\begin{center}
\adjustbox{max width=0.6\linewidth}{%
\begin{tikzpicture}[scale=0.55, every node/.style={draw,rounded corners,align=center,font=\tiny}]
\node[fill=violet!15] (data) at (0,3) {Data};
\node (t1) at (-1.8,1.5) {Tree 1};
\node (t2) at (0,1.5) {Tree 2};
\node (t3) at (1.8,1.5) {Tree N};
\node[fill=violet!15] (vote) at (0,0) {Vote / Average};
\draw[->] (data) -- (t1);
\draw[->] (data) -- (t2);
\draw[->] (data) -- (t3);
\draw[->] (t1) -- (vote);
...
\end{center}
Bootstrap samples $\to$ independent trees $\to$ majority vote / average
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest Algorithm: Final Classifier}
...
\item For regression problems, we usually take $m = \frac{d}{3}$, where $d$ is the number of features.
\item Recommended leaf size: $n_\text{min} = 1$ example for classification, $n_\text{min} = 5$ examples for regression.
...
$a(x)$ is just an average (or vote) over $N$ trees, and averaging cancels out each tree's
individual noise, which is why more (decorrelated) trees keep reducing variance. Restricting
each split to a random subset of $m$ features is what decorrelates the trees in the first
place: without it, all $N$ trees would keep picking the same strongest feature first and end
up highly similar, and averaging similar things doesn't reduce variance nearly as much.
\end{block}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Ensemble \& Random Forest}
\begin{block}{Think About It}
There are 7 jurors in the courtroom. Each of them individually can correctly determine whether the defendant is guilty or not with 80\% probability. How likely is the jury to make a correct verdict jointly if the decision is made by majority voting?
\begin{enumerate}
\item[A.] 20.97\%
\item[B.] 80.00\%
\item[C.] 83.70\%
...
\begin{frame}[fragile]\frametitle{Quick Check: Ensemble \& Random Forest}
\begin{block}{Answer}
\textbf{Correct: D.} With 7 jurors at $p=0.8$ each, the jury is correct whenever 4 or more
agree: $\sum_{k=4}^{7}{7 \choose k}(0.8)^k(0.2)^{7-k} \approx 0.9666$, higher than any
single juror's 80\%. This illustrates the same jury-theorem effect as the ensemble formula shown earlier.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest: Pros (1/2)}
Pros
\begin{itemize}
...
\item Doesn't require fine-grained parameter tuning; works well out-of-the-box.
\begin{itemize}
\item With tuning, gains of about 0.5--3\% accuracy are possible, depending on the problem.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest: Pros (2/2)}
Pros
\begin{itemize}
\item Rarely overfits.
\begin{itemize}
\item More trees almost always help.
\item But gains flatten out (the learning curve approaches an asymptote) after enough trees.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest: Cons}
Cons
\begin{itemize}
...
\item Performs worse than linear methods in the case of sparse data: text inputs, bag of words, etc.
\item Unlike linear regression, Random Forest cannot extrapolate.
\begin{itemize}
\item This can also be an advantage: outliers don't cause extreme predicted values.
\end{itemize}
\item For categorical variables with varying numbers of levels, random forests favor variables with more levels.
\begin{itemize}
\item The tree fits more towards high-level features, since that gains more accuracy.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest (Recap): What It Is}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest (Recap): How It Works}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Random Forest (Recap): Feature Ranking}
\begin{itemize}
