...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: Generators}
\begin{block}{Intuition}
A generator function is like a lazy vending machine: it only produces the next item when you
press the button (\lstinline|next()|), rather than manufacturing and storing the whole product
line up front the way a list would. That makes generators the natural fit for sequences too
large -- or infinite -- to hold in memory all at once.
\end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Generator Usage}
Each time the yield statement is executed the function generates a new value. A generator is
also an iterator, so you don't have to worry about the iterator protocol:
\begin{lstlisting}
>>> y = yrange(3)
>>> y
...
\begin{frame}[fragile]\frametitle{Generator Examples}
    \begin{itemize}
    \item  We mostly use generators for \textbf{lazy evaluations}.
...
\item A classic example is \lstinline|os.walk()| --- a generator that lazily traverses directory trees without loading the whole tree into memory.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Generator Example}
The following example demonstrates the interplay between \lstinline|yield| and calls to \lstinline|next()| on a generator object.
\begin{lstlisting}
>>> def foo():
...     print("begin")
...     for i in range(3):
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{\texttt{yield from}: Delegating to a Sub-generator}
When a generator's job is mostly to hand off to another generator, \lstinline|yield from| avoids
writing a manual \lstinline|for ... yield| loop, and forwards the sub-generator's return value
too:
\begin{lstlisting}
def readfiles(filenames):
    for f in filenames:
        with open(f) as fh:
            yield from fh   # same as: for line in fh: yield line
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Example: Reading multiple files}
The code is much simpler now with each function doing one small thing. We can move all these
functions into a separate module and reuse it in other programs:
\begin{lstlisting}
...
\lstinline|chain|: chains multiple iterators together.
\begin{lstlisting}
>>> it1 = iter([1, 2, 3])
>>> it2 = iter([4, 5, 6])
>>> list(itertools.chain(it1, it2))  # chain returns an iterator
[1, 2, 3, 4, 5, 6]
\end{lstlisting}
In Python 3, \lstinline|zip()| is built-in and already lazy (\lstinline|itertools.izip| was removed):
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Generators}
\begin{block}{Think About It}
\lstinline|squares()| calls \lstinline|integers()|, an infinite generator. Why doesn't
\lstinline|print(take(5, squares()))| hang forever?
\end{block}
\pause
\begin{block}{Answer}
Because generators are lazy: \lstinline|integers()| doesn't produce any values until something
asks for them via \lstinline|next()|. \lstinline|take()| only calls \lstinline|next()| 5 times
(via its \lstinline|for i in range(n)| loop), so only 5 values ever get pulled through the whole
chain -- the ``infinite'' sequence is never actually materialized.
\end{block}
\end{frame}
