...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Conditionals}
  Conditional execution uses the \texttt{if} statement. The \texttt{elif} can be repeated, with different conditions, or left out entirely; the \texttt{else} clause is optional too. Where's the `end if'? There's no `end if': indentation delimits blocks!
\begin{lstlisting}
if expr:
  # indented block
elif  other-expr:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{switch/case and match/case}
  \begin{itemize}
\item Python $<$ 3.10 had no switch/case; use chained \texttt{if/elif/else}.
\item Since \textbf{Python 3.10}, the \texttt{match}/\texttt{case} statement is available:
\end{itemize}
\begin{lstlisting}
command = "quit"
match command:
    ...
    case _:      print("Unknown command")  # default
\end{lstlisting}
\begin{block}{Intuition}
Think of \texttt{match}/\texttt{case} as a signpost with labelled roads: Python checks \texttt{command} against each \texttt{case} label in order and follows the first one that matches, falling through to \texttt{case \_} (the ``anything else'' road) if nothing else fits.
\end{block}
\textbf{Alternative (all Python versions)} --- dictionary of functions:
\begin{lstlisting}
result = {
	'a': lambda x: x * 5,
	'b': lambda x: x + 7,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Conditionals}
\begin{block}{Think About It}
\texttt{n\%2 == 0 or n\%3 == 0} is true if a number is divisible by 2 or 3. If you swapped \texttt{or} for \texttt{and}, would the expression ever be true for a whole number? Which numbers, if any, satisfy \texttt{n\%2==0 and n\%3==0}?
\end{block}
\pause
\begin{block}{Answer}
Yes, plenty: any number divisible by \emph{both} 2 and 3, i.e. any multiple of 6 (6, 12, 18, ...). It highlights the difference between \texttt{or} (true if either side is true -- a wide net) and \texttt{and} (true only if both sides are true -- a narrow, stricter net).
\end{block}
\end{frame}
