...
\begin{itemize}
\item Each entity table is represented by a label on nodes
\item Each row in a entity table is a node
\item Columns on those tables become node properties.
\item Join tables are transformed into relationships, columns on those tables become relationship properties

\item  Drop the WHERE clause; Adopt the MATCH clause
\end{itemize}
