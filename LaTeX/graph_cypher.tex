...
MATCH (a:Person)-[:ACTED_IN]->(m:Movie)
WHERE a.name = actorName AND toLower(m.title) CONTAINS mt
...
\begin{lstlisting}
// Neo4j 4.4+ syntax: uniqueness constraint
CREATE CONSTRAINT FOR (label:Label) REQUIRE label.property IS UNIQUE

// Fails if multiple nodes already share the same property value.

// Constraint on a relationship property (direction not required):
CREATE CONSTRAINT FOR ()-[rel:REVIEWED]-() REQUIRE rel.rating IS NOT NULL
\end{lstlisting}	  
