...
\begin{itemize}
\item Imagine you stepped out of a time machine from the 1960s or 1970s
\end{itemize}
...
\item No ambiguity
\item Described by context-free grammars; parsed by compiler tools
...
\begin{itemize}
\item Can a machine truly understand language the way humans do?
\item \textbf{AI-Complete}: as hard as the hardest problems in AI --- solving NLP fully requires solving all of AI
\item Understanding language requires world knowledge, reasoning, context, and common sense
\item Use of language is the touchstone of intelligent behavior
\item \textbf{The Turing Test (Alan Turing, 1950)}: a human judge converses with two parties (one human, one machine) --- if the judge cannot reliably tell them apart, the machine passes
\end{itemize}
\begin{center}
\includegraphics[width=0.35\linewidth,keepaspectratio]{turing}
\end{center}
\end{frame}
...
\item Annual Turing Test competition, held 1991--2020
\item \$100,000 for first bot indistinguishable from human (never awarded)
\item Competition discontinued after 2020 due to lack of funding
\item Notable winners: Mitsuku chatbot (2013, 2016--2019)
\end{itemize}
\begin{center}
\includegraphics[width=0.25\linewidth,keepaspectratio]{mitsuku}
\end{center}
The prize history shows how hard human-level language understanding remains.
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Transformers: The Key Intuition}
\textbf{Why did Transformers change everything?}
\begin{itemize}
\item Earlier models (RNNs) read text \textit{one word at a time} --- like reading left-to-right with a fixed-size memory
\item \textbf{Attention mechanism}: every word can ``look at'' every other word simultaneously
\begin{itemize}
\item In ``The bank by the river was steep'', \textit{bank} attends to \textit{river} to resolve its meaning
\end{itemize}
\item \textbf{Parallelisation}: all positions processed at once $\Rightarrow$ massive speedup on GPUs
\item \textbf{Transfer learning}: pre-train once on huge text corpora; fine-tune cheaply on any downstream task
\end{itemize}
\vspace{0.3cm}
Result: one architecture now dominates machine translation, Q\&A, summarisation, code generation, and beyond.
\end{frame}
...
\end{itemize}
\vspace{0.3cm}
\textit{Humans like words, computers like numbers --- bridging this gap is the central challenge of NLP.}
\end{frame}


\section[Appl]{NLP Concepts \& Applications}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{}
\begin{center}
...
{\small (B\,=\,Begin entity,\; I\,=\,Inside entity,\; O\,=\,Outside any entity)}
...
\begin{itemize}
\item Subword tokenization: BPE (Byte-Pair Encoding --- merges frequent character pairs into subword units), WordPiece (similar, used by BERT)
\item SentencePiece for multilingual models
\item Handles out-of-vocabulary words better than word-level tokenization
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{center}
\includegraphics[width=0.6\linewidth,keepaspectratio]{postag}
\end{center}
...
\item Claude 3.7 Sonnet / Claude 4 (Anthropic)
...
\begin{itemize}
\item NLP makes computers understand and generate human language
\item Language is ambiguous, context-dependent --- \textit{hard for machines}
\item Humans like words; computers like numbers --- we bridge this via \textbf{representation learning}
\item Four approaches: rules $\to$ ML $\to$ deep learning $\to$ LLMs
\item Applications everywhere: search, translation, assistants, analysis
\item Exciting time --- rapid innovation continues!
\end{itemize}
\end{column}
