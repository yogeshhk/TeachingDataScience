...
In the Glass Identification example, \texttt{predict\_proba(X)[:, 1]} is used to get the predicted probability of class 1 (household glass). Why index with \texttt{[:, 1]} instead of just calling \texttt{logreg.predict(X)} and treating the 0/1 output as a probability?
\end{block}
\pause
\begin{block}{Answer}
