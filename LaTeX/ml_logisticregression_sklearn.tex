...
# Use only petal length (col 2) and petal width (col 3) as features
X = iris.data[:, [2, 3]]

# Target: integer class labels (0=setosa, 1=versicolor, 2=virginica)
y = iris.target
\end{lstlisting}
...
Logistic regression optimizes its coefficients with gradient-based methods, and features on very different scales (e.g. petal length in cm vs. a count in the thousands) make that optimization landscape lopsided -- steep in one direction, flat in another -- which slows or destabilizes convergence. Scaling every feature to comparable range/variance turns that landscape into something closer to a bowl, which is faster and more reliable to descend.
\end{block}
...
lr.fit(X_train_std, y_train)  # use the scaled features from the previous step
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Logistic Regression with Scikit-Learn}
\begin{block}{Think About It}
In the Glass Identification example, \texttt{logreg.predict\_proba(X)[:, 1]} is used to get the predicted probability of class 1 (household glass). Why index with \texttt{[:, 1]} instead of just calling \texttt{logreg.predict(X)} and treating the 0/1 output as a probability?
\end{block}
\pause
\begin{block}{Answer}
\texttt{predict()} collapses the model's confidence down to a single hard label by thresholding internally (at 0.5 by default) -- you lose the information of \emph{how} confident the model was. \texttt{predict\_proba()} returns both class probabilities per row (columns sum to 1), so \texttt{[:, 1]} keeps the full, continuous confidence score for class 1 -- letting you see a borderline 0.51 differently from a confident 0.98, or apply a different decision threshold than 0.5 if the cost of false positives and false negatives isn't symmetric.
\end{block}
\end{frame}
