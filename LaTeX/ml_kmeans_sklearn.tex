...
ax.scatter(X[:, 3], X[:, 0], X[:, 2],
          c=labels.astype(float), edgecolor="k", s=50)
...
\begin{lstlisting}
import matplotlib.pyplot as plt
def plot_clusters(orig,pred,nx,ny,*args):
	data = orig
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: K-Means with Scikit-Learn}
\begin{block}{Think About It}
In the Beer clustering example, the features -- calories, sodium, alcohol, cost -- are on
very different numeric scales. Since k-means uses Euclidean distance to decide cluster
membership, what problem could this cause, and how would you fix it before clustering?
\end{block}
\pause
\begin{block}{Answer}
Whichever feature has the largest raw magnitude (likely calories, often in the hundreds
while alcohol is a single digit) will dominate the distance calculation and effectively
decide the clusters on its own, drowning out the other features -- the same scaling problem
seen with KNN. The standard fix is to standardize each feature (zero mean, unit variance)
before running \texttt{KMeans}, so every feature contributes comparably to the distance.
\end{block}
\end{frame}
