...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: date, time, datetime, timedelta}
\begin{block}{Intuition}
Think of \texttt{date} as a page torn from a calendar (just day info, no clock), \texttt{time}
as a clock face with no date attached, and \texttt{datetime} as one specific tick of a clock on
one specific calendar page -- both combined. A \texttt{timedelta} isn't a point in time at all;
it's a stopwatch reading, the \emph{gap} between two such points.
\end{block}
\end{frame}
...
for m in [ 1, 0, 0.1, 0.6 ]:
    try:
        print('%02.1f :' % m, datetime.time(0, 0, 0, microsecond=int(m)))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Date/Time Arithmetic}
\begin{block}{Think About It}
Why does \lstinline|d1 - d2| between two \texttt{date} objects give you a \texttt{timedelta},
but \lstinline|d1 - timedelta(days=1)| gives you back a \texttt{date}?
\end{block}
\pause
\begin{block}{Answer}
Subtracting two \emph{points} in time (\texttt{date} $-$ \texttt{date}) tells you the
\emph{duration} between them -- hence a \texttt{timedelta}. Subtracting a \emph{duration} from a
point (\texttt{date} $-$ \texttt{timedelta}) shifts that point backward by that span, landing on
another point in time -- hence a \texttt{date}. Same operator, different meaning, depending on
whether both operands are points or one is a point and the other a span -- just like
``location $-$ location = distance'', but ``location $-$ distance = another location''.
\end{block}
\end{frame}
