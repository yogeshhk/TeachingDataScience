...
\item Heavily use recommendations/suggestions to users. 3/4th of revenues.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}\frametitle{The Story: Cambridge Analytica}
...
real time as customers make purchases. Would AWS Lambda's ``trigger only when a condition
is met'' model or a continuously-running EC2 server be the better architectural fit, and why?
\end{block}
\end{frame}
...
\begin{frame}[fragile]\frametitle{Quick Check: Production and Deployment}
\begin{block}{Answer}
Lambda fits well here: fraud-checking is a short-lived, event-driven task (react to each
transaction as it happens) rather than something needing a server running and warmed up
24/7, so a serverless trigger avoids paying for idle capacity and scales automatically with
transaction volume. EC2 suits models that need to stay loaded in memory and serve requests
continuously at low latency, such as a recommendation engine backing every page load.
\end{block}
\end{frame}
