...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{General Form: Exact Function}
...
Think of $Y$ as the thing you want to predict (say, a house's price) and $X_1,\ldots,X_p$ as the inputs you already know (size, location, age). $F$ is simply the rule that turns those inputs into a prediction, if such an exact rule existed.
\end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{General Form: With Error Term}
...
\end{itemize}
\begin{block}{Intuition}
In practice we never find the perfect $F$ from the previous slide, only an approximation $f$. The gap between what $f$ predicts and reality is $\epsilon$, unpredictable noise (measurement quirks, factors we didn't record). Statistical learning is just the toolbox for finding a good $f$ from data.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Estimating $f$: Two Approaches}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Estimating $f$: Parametric vs Non-Parametric}
\begin{itemize}
\item Parametric:
	\begin{itemize}
...
\end{itemize}
\begin{block}{Intuition}
Parametric is like assuming the relationship is a straight line and just needing to find its slope and intercept, a handful of numbers ($\beta$'s) to tune. Non-parametric makes no such assumption and lets the data itself trace out whatever curve fits best, more flexible, but it needs a lot more data to do so reliably.
\end{block}
\end{frame}
...
\item $RSS = \sum_{i=1}^n e_i^2$ , where $e_i = y_i - \hat{y}_i$ (actual minus predicted)
\end{itemize}
\begin{block}{Intuition}
This is just ``fitting a line (or plane) through points.'' The $\beta$'s are the slope and intercept, adjusting them tilts and shifts the line. RSS measures how far off the line's predictions are from the real points, squared so bigger misses count more, and the best-fit line is the one that makes this total miss as small as possible.
\end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{How to Find Best Fit: Matrix Form}
Matrix Form:
\begin{itemize}
\item  We can solve the closed-form equation for coefficient vector $w$: $w = (X^T X)^{-1} X^T Y$.
	\begin{itemize}
	\item $X$: matrix of input data (each row is one data point, each column is one predictor); $Y$: column of output values
	\item $X^T$: transpose of $X$ (its rows and columns swapped)
	\item $X^T X$: a square matrix summarizing how the predictors relate to each other
	\item $(X^T X)^{-1}$: the ``inverse'' of that matrix, it undoes it, much like dividing by a number
	\item $X^T Y$: summarizes how each predictor relates to the output $Y$
	\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{How to Find Best Fit: Gradient Descent}
\adjustbox{valign=t}{%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: The Math Framework}
\begin{block}{Think About It}
The general form $Y = f(X) + \epsilon$ includes an irreducible error term $\epsilon$ that no model, however good, can eliminate. If you had a perfect model that estimated $f$ exactly, would your predictions ever be 100\% accurate?
\end{block}
\end{frame}
...
\begin{frame}[fragile]\frametitle{Quick Check: The Math Framework}
\begin{block}{Answer}
No, even with a perfect estimate of $f$, the $\epsilon$ term (random noise independent of $X$) still contributes to prediction error. This is why statistical learning aims to minimize the reducible error (the gap between $\hat{f}$ and the true $f$), not to eliminate all error entirely.
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Cross Validation}
\begin{block}{Think About It}
...
\begin{frame}[fragile]\frametitle{Practical Tip}
	\begin{itemize}
		\item Cross Validation is NOT used for Model Building, but for choosing the best model/hyperparameters amongst several choices (Linear model vs Decision Tree vs SVM, or $k=3$ vs $k=11$ for KNN).
		\item Once a specific model with known hyperparameters is finalized using the least average error, a new model is built using the FULL training data.
		\item No further splits are done at that final stage.
	\end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Accuracy (Regression): Mean Squared Error}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Accuracy (Classification)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Recall (Sensitivity or True positive rate)}
...
\begin{frame}[fragile]\frametitle{Sensitivity vs Specificity}
	...
	\begin{itemize}
	\item For sensitivity we look at only the Positives group (all predicted positive); the rest fall into the negatives group.
...
\begin{center}
\includegraphics[width=0.7\linewidth,keepaspectratio]{confmat9}
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Precision: Intuition}
\begin{block}{Fishing for a specific fish}
You are out on the lake, but you only want rainbow trout. At the end of the day you look
into your bucket and ask: of the fish I actually kept, what fraction were really rainbow
trout?
\end{block}
	\begin{itemize}
	\item That fraction is precision. The denominator is what you \textbf{claimed}, not what exists.
	\item It says nothing about the trout still swimming in the lake; those never entered the calculation.
	\item So precision punishes False Positives: every carp in your bucket drags it down.
	\item Use a finer, more selective net (a stricter threshold) and precision goes up.
	\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Recall (Coverage of actual positives)}
Recall (REC) is calculated as the number of correct positive predictions divided by the
total number of \textbf{actual} positives. This is the same quantity as sensitivity seen
earlier; it is worth restating here, now that precision is on the table, because the two
are always read as a pair.

\begin{center}
\includegraphics[width=0.7\linewidth,keepaspectratio]{confmat7}
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Recall: Intuition}
\begin{block}{Casting the net}
Same lake, different question: of all the fish that were in the lake to begin with, what
fraction ended up in my net?
\end{block}
	\begin{itemize}
	\item That fraction is recall. The denominator is what \textbf{exists}, not what you claimed.
	\item Fish that got away are False Negatives, so recall punishes misses.
	\item Cast a wider net (a relaxed threshold) and recall goes up; in the limit, net the whole lake and recall is 1.0.
	\item But that same wide net hauls in boots and weeds, so precision falls. Hence the trade-off.
	\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Precision vs Recall: One Net, Two Questions}
		\begin{center}
		\small
		\adjustbox{max width=\linewidth}{
			\begin{tabular}{| p{0.26\linewidth} | p{0.29\linewidth} | p{0.29\linewidth} |}
				...
				Question & Of what I caught, how much was right? & Of what was there, how much did I catch? \\ \hline
				Formula & $\frac{TP}{TP+FP}$ & $\frac{TP}{TP+FN}$ \\ \hline
				Denominator & What the model claimed & What actually exists \\ \hline
				Punishes & False Positives (false alarms) & False Negatives (misses) \\ \hline
				Improved by & Stricter threshold, finer net & Relaxed threshold, wider net \\ \hline
				Care when & A wrong flag is expensive: spam filter, arrest, costly recall of parts & A miss is expensive: cancer screening, fraud, crack detection \\ \hline
			\end{tabular}
			}
\end{center}
...
Type I Error is equivalent to False Positive (FP).
	\begin{itemize}
	\item Test is Positive but actually disease is not there.
...
Type II Error is equivalent to False Negative (FN). 
	...
	\begin{itemize}
	\item Test is Negative but actually disease is there.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Why Harmonic Mean and not the Others?}
Three models, three ways of averaging their precision and recall:
		...
		\adjustbox{max width=\linewidth}{
			\begin{tabular}{| c | c | c | c | c |}
				\hline
				\textbf{P} & \textbf{R} & \textbf{Arithmetic} & \textbf{Geometric} & \textbf{Harmonic (F1)} \\ \hline
				0.5 & 0.5 & 0.50 & 0.50 & 0.50 \\ \hline
				0.9 & 0.1 & 0.50 & 0.30 & 0.18 \\ \hline
				1.0 & 0.0 & 0.50 & 0.00 & 0.00 \\ \hline
			\end{tabular}
			}
\end{center}

	\begin{itemize}
	\item The arithmetic mean scores all three rows at 0.50. It cannot tell a genuinely balanced model from one that is useless on one side.
	\item Row 3 is the giveaway: a model with zero recall still scores 0.50. Averaging additively lets a strong metric pay for a failed one.
	\item The harmonic mean collapses towards the weaker number, so it is 0.18 and 0.00 for the last two rows. F1 is high only when \textbf{both} are high.
	\end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Why Harmonic Mean: The Reasoning}
	\begin{itemize}
	\item Precision and recall are \textbf{rates with different denominators} ($TP+FP$ versus $TP+FN$). The harmonic mean is the standard way to average rates, because it averages the denominators rather than the ratios.
	\item It is deliberately pessimistic. Being the reciprocal of the average reciprocal, one small value blows up its reciprocal and drags the whole result down.
	\item That pessimism is the point. It blocks the cheap way to game the metric: flag every sample positive and recall becomes 1.0 while precision drops to the base rate. Arithmetic mean rewards that trick, F1 does not.
	\item The geometric mean also punishes imbalance and reaches 0 in the extreme, but harmonic punishes harder in the middle (0.18 versus 0.30 above), so it is the convention.
	\item Order holds throughout: harmonic $\leq$ geometric $\leq$ arithmetic, with equality only when precision and recall are equal.
	\end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Which One Should You Optimize?}
\begin{itemize}
\item Precision addresses False Positives and Recall addresses False Negatives, so the choice comes down to which of the two errors is more expensive for your problem.
\item If you are hunting something rare (skewed) like Cancer detection or Fraud, you are typically concerned with FN, because the test must not wrongly clear someone who actually has the disease. A missed case is far costlier than a follow-up test on a false alarm. Use Recall.
\item When a false alarm itself is expensive or disruptive, such as a spam filter deleting a genuine mail, or scrapping a good part on the shop floor, use Precision.
\item When both errors matter and you need a single number to compare models, use F1.
\end{itemize}
\end{frame}
...
% "Precision and Recall: A Tug of War" removed here (Aug 2026 /upgrade-deck
% pass) -- its 3 bullets (precision/recall oppose each other, need both) added
% nothing beyond the "Precision and Recall trade-off" frame right after, which
% explains the actual mechanism (threshold relaxation) rather than just
...
\begin{center}
\includegraphics[width=0.32\linewidth,keepaspectratio]{ovefit1}%
\hspace{0.03\linewidth}%
\includegraphics[width=0.32\linewidth,keepaspectratio]{ovefit2}
\end{center}
	\begin{itemize}
...
\tiny{(Ref: Why Is Over-fitting Bad in Machine Learning? - StackOverflow)}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Regularization: Intuition}
	\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Regularization: Formal Definition}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Generalization}
\begin{block}{Think About It}
...
% Stale "Evaluation Metrics: Prediction Bias" divider removed here (Aug 2026
% /upgrade-deck pass) -- the frames it actually introduced (avg-prediction-vs-
% avg-actual calibration, calibration plot) are all commented out below, so it
% no longer matches what follows (bias/variance error decomposition instead);
% the "Bias and Variance" divider right above already covers that content.
...
% Uncommented (Aug 2026 /upgrade-deck pass): the "Intuition" walkthrough was
% jumping straight from the train/test setup (bv3) to the "Ideal" resolution
% (bv8), skipping the actual bias example and variance example that give the
% resolution its meaning -- these 4 frames restore that missing middle.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% "Learning Method Bias" / "Learning Method Variance" removed here (Aug 2026
% /upgrade-deck pass) -- both re-defined Bias and Variance from scratch using
% nearly the same wording as the earlier "Bias"/"Variance" frames, and came
% positionally after the trade-off frames had already used both terms without
% redefinition. The StatQuest "Intuition" walkthrough (bv1-bv8) already teaches
% and explicitly names both terms concretely and memorably in between, so this
% was a third, redundant pass over the same two definitions.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Bias-Variance Trade-Off}
\begin{block}{Think About It}
