...
2nd layer onwards, no need to specify input\_shape. You may keep/change the number of units. Last LSTM has return\_sequences to False.
\begin{lstlisting}
...
Output is the stock prediction, which is dimension 1, so output layer's dim is 1.
...
Last thing is that for reproducibility, a seed is used in numpy's random.
\begin{lstlisting}
import matplotlib.pyplot as plt
import numpy as np
import time
