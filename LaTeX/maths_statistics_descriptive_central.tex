...
Picture ten people in a room, then a billionaire walks in. The mean income skyrockets (it "feels" every value), the median barely budges (it only cares about the middle), and the mode doesn't move at all (nobody's income repeated). That's why median is the go-to when outliers are a concern.
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Central Tendency}
\begin{block}{Think About It}
A dataset's mean is noticeably higher than its median. What does that suggest about the shape of the distribution?
\end{block}
\pause
\begin{block}{Answer}
It suggests the data is right-skewed (a long tail of high values pulling the mean upward), since the mean is sensitive to extreme values while the median is not.
\end{block}
\end{frame}
