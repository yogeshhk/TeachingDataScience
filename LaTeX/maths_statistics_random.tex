...
Don't picture a "random variable" as something that changes value on its own. It's just a fixed translator: it takes a messy real-world outcome (heads/tails, a die face) and assigns it a number, once and for all, so we can finally do arithmetic and calculus on what happened.
\end{block}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: Random Variables}
\begin{block}{Think About It}
Is "number of days it rains in a year" a discrete or continuous random variable? What about "amount of rainfall on a given day"?
\end{block}
\pause
\begin{block}{Answer}
"Number of days" is discrete -- it can only take whole-number values (0, 1, 2, ...). "Amount of rainfall" is continuous -- it can take any real value in a range (e.g. 2.37 mm), since rainfall can be measured to arbitrary precision.
\end{block}
\end{frame}
