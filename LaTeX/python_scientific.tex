...
\begin{frame}[fragile]
    ...
    Never do this -- it sends 10 separate jobs to the underlying C library:
    \begin{lstlisting}
>>> for i in range(10):
        z[i] = x[i] + y[i]
    \end{lstlisting}

    Always try to do vectorized calculations instead -- this sends only one job:
    \begin{lstlisting}
>>> z = x+y
    \end{lstlisting}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Intuition: Broadcasting}
\begin{block}{Intuition}
Broadcasting is like stamping a single row of a pattern across every row of a grid, without
actually copying that row in memory: if one array has shape \texttt{(4,)} and the other
\texttt{(3,4)}, NumPy conceptually stretches (``broadcasts'') the smaller one to match the
bigger one's shape before applying the operation element-by-element -- as long as the trailing
...
# Use np.concatenate for higher dimensions:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Quick Check: NumPy \& SciPy}
\begin{block}{Think About It}
Given \lstinline|x = np.arange(5)| and \lstinline|y = x| (not \lstinline|np.copy(x)|), why does
changing \lstinline|x[0]| also change \lstinline|y[0]|? How would you avoid this?
\end{block}
\pause
\begin{block}{Answer}
NumPy arrays are mutable, and \lstinline|y = x| doesn't copy the array -- it just makes
\texttt{y} a second reference to the same underlying memory. Use \lstinline|y = np.copy(x)| (or
\lstinline|x.copy()|) to get an independent array that won't be affected by changes to \texttt{x}.
\end{block}
\end{frame}
