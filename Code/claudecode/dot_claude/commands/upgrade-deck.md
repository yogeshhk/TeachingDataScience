...
### Step 0: Automated Audit

Before reading anything by hand, run the `latex-audit` skill on the driver:
...
you can't fully see. `headings` returns two **candidate** lists (reorder-hostile manual
numbering in frame titles, and frame titles that redundantly restate the deck's own subject) --
these are pattern-matches, not verdicts, so weigh each one the way `latex-audit`'s own SKILL.md
describes (canonical sequences, exam-blueprint numbers, and legitimate cross-framework mentions
are known false positives) before folding any of them into your findings below.
